...
if not [%BUILD_TOOLSET%] == [] (
    cmake -A %BUILD_ARCH% -T %BUILD_TOOLSET% -D%BUILD_TARGET%=1 -DFONLINE_CMAKE_CONTRIBUTION=%FO_CMAKE_CONTRIBUTION% -DFONLINE_UNIT_TESTS=0 "%FO_ROOT%"
) else if [%BUILD_CACHE%] == [] (
    cmake -A %BUILD_ARCH% -D%BUILD_TARGET%=1 -DFONLINE_CMAKE_CONTRIBUTION=%FO_CMAKE_CONTRIBUTION% -DFONLINE_UNIT_TESTS=0 "%FO_ROOT%"
) else (
    cmake -A %BUILD_ARCH% -C "%FO_ROOT%\BuildTool\%BUILD_CACHE%" -D%BUILD_TARGET%=1 -DFONLINE_CMAKE_CONTRIBUTION=%FO_CMAKE_CONTRIBUTION% -DFONLINE_UNIT_TESTS=0 "%FO_ROOT%"
)
