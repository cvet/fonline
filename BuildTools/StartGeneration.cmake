...
DeclareOption(FO_EMBEDDED_DATA_CAPACITY "Capacity for embedded data in binaries" 200000)
...
if(MSVC AND NOT CMAKE_CXX_COMPILER_ID MATCHES "Clang")
	...
	set(expr_SanitizerConfigs $<CONFIG:San_Address,Debug_San_Address>)
...
set(expr_DebugBuild $<CONFIG:Debug,Debug_Profiling_Total,Debug_Profiling_OnDemand,Debug_San_Address>)
set(expr_FullOptimization $<CONFIG:Release_Ext>)
set(expr_WebFullOptimization $<CONFIG:Release,Release_Ext>)
set(expr_DebugInfo $<NOT:$<CONFIG:MinSizeRel>>)
set(expr_WebDebugInfo $<NOT:$<CONFIG:Release,Release_Ext,MinSizeRel>>)
set(expr_PrefixConfig $<NOT:$<CONFIG:Release,RelWithDebInfo,MinSizeRel,Release_Ext>>)
set(expr_TracyEnabled $<CONFIG:Profiling_Total,Profiling_OnDemand,Debug_Profiling_Total,Debug_Profiling_OnDemand>)
set(expr_TracyOnDemand $<CONFIG:Profiling_OnDemand,Debug_Profiling_OnDemand>)
...
add_compile_definitions($<${expr_DebugBuild}:DEBUG>)
add_compile_definitions($<${expr_DebugBuild}:_DEBUG>)
add_compile_definitions($<${expr_DebugBuild}:FO_DEBUG=1>)
add_compile_definitions($<$<NOT:${expr_DebugBuild}>:NDEBUG>)
add_compile_definitions($<$<NOT:${expr_DebugBuild}>:FO_DEBUG=0>)
...
add_compile_definitions(FO_NO_EXTRA_ASSERTS=0) # Todo: FO_NO_EXTRA_ASSERTS=$<CONFIG:Release_Ext> for first need separate asserts from handled errors
add_compile_definitions(FO_NO_TEXTURE_LOOKUP=0) # Todo: FO_NO_TEXTURE_LOOKUP need option for enable
add_compile_definitions(FO_DIRECT_SPRITES_DRAW=0) # Todo: FO_DIRECT_SPRITES_DRAW need option for enable
add_compile_definitions(FO_RENDER_32BIT_INDEX=0) # Todo: FO_RENDER_32BIT_INDEX need option for enable
...
if(FO_BUILD_SERVER OR FO_BUILD_MAPPER OR FO_BUILD_EDITOR OR FO_BUILD_ASCOMPILER OR FO_BUILD_BAKER OR FO_UNIT_TESTS OR FO_CODE_COVERAGE)
...
if(WIN32)
	...
	add_compile_options_C_CXX($<$<NOT:${expr_DebugBuild}>:/sdl->)
	...
	if(FO_BUILD_CLIENT)
		add_compile_options_C_CXX($<${expr_DebugBuild}:/MTd>)
		add_compile_options_C_CXX($<$<NOT:${expr_DebugBuild}>:/MT>)
	else()
		add_compile_options_C_CXX($<${expr_DebugBuild}:/MDd>)
		add_compile_options_C_CXX($<$<NOT:${expr_DebugBuild}>:/MD>)
	endif()

	list(APPEND FO_ESSENTIALS_SYSTEM_LIBS "user32" "ws2_32" "version" "winmm" "imm32" "dbghelp" "psapi" "xinput")
...
elseif(CMAKE_SYSTEM_NAME MATCHES "Android")
	...
	list(APPEND FO_ESSENTIALS_SYSTEM_LIBS "android" "log" "atomic")
	add_compile_options_C_CXX($<${expr_DebugInfo}:-g>)
	add_compile_options_C_CXX($<${expr_FullOptimization}:-O3>)
	add_compile_options_C_CXX($<${expr_FullOptimization}:-flto>)
	add_link_options($<${expr_FullOptimization}:-flto>)
...
elseif(CMAKE_SYSTEM_NAME MATCHES "Emscripten")
	...
	add_compile_options_C_CXX($<${expr_WebDebugInfo}:-g3>)
	add_link_options($<${expr_WebDebugInfo}:-g3>)
	add_compile_options_C_CXX($<${expr_WebFullOptimization}:-O3>)
	add_link_options($<${expr_WebFullOptimization}:-O3>)
	add_compile_options_C_CXX($<${expr_WebFullOptimization}:-flto>)
	add_link_options($<${expr_WebFullOptimization}:-flto>)
	add_link_options($<IF:${expr_DebugBuild},-O0,-O2>)
	add_link_options(-sASSERTIONS=$<IF:${expr_DebugBuild},2,0>)
	add_link_options(-sSTACK_OVERFLOW_CHECK=$<IF:${expr_DebugBuild},2,0>)
