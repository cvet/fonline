...
var FosRuntime = class extends import_events.EventEmitter {
...
var DEFAULT_DISCOVERY_PORT = 43001;
var DEFAULT_DISCOVERY_TIMEOUT_MS = 800;
...
var ATTACH_STOP_HISTORY_LIMIT = 8;
async function connectAttachTransport(endpoint) {
  ...
  return await new Promise((resolve, reject) => {
    const socket = resolved.kind === "tcp" ? net.createConnection({ host: resolved.host, port: resolved.port }) : net.createConnection({ path: resolved.path });
    let settled = false;
    let nextRequestId = 1;
    ...
    const finishResolve = () => {
      ...
      resolve({
        transport: {
          ...
          sendRequest: async (command, payload) => {
...
var FosDebugSession = class _FosDebugSession extends import_debugadapter.LoggingDebugSession {
  /**
  ...
  constructor(fileAccessor) {
    ...
    this._pendingAttachBreakpoints = /* @__PURE__ */ new Map();
    this._variableHandles = new import_debugadapter.Handles();
    this._attachScopeRefs = /* @__PURE__ */ new Map();
    ...
    this._runtime.on("stopOnEntry", () => {
      this.sendEvent(new import_debugadapter.StoppedEvent("entry", _FosDebugSession.threadID));
    });
    this._runtime.on("stopOnStep", () => {
      this.sendEvent(new import_debugadapter.StoppedEvent("step", _FosDebugSession.threadID));
    });
    this._runtime.on("stopOnBreakpoint", () => {
      this.sendEvent(new import_debugadapter.StoppedEvent("breakpoint", _FosDebugSession.threadID));
    });
    this._runtime.on("stopOnDataBreakpoint", () => {
      this.sendEvent(new import_debugadapter.StoppedEvent("data breakpoint", _FosDebugSession.threadID));
    });
    this._runtime.on("stopOnInstructionBreakpoint", () => {
      this.sendEvent(new import_debugadapter.StoppedEvent("instruction breakpoint", _FosDebugSession.threadID));
    });
    this._runtime.on("stopOnException", (exception) => {
      if (exception) {
        this.sendEvent(new import_debugadapter.StoppedEvent(`exception(${exception})`, _FosDebugSession.threadID));
      } else {
        this.sendEvent(new import_debugadapter.StoppedEvent("exception", _FosDebugSession.threadID));
      }
    });
...
`;
  ...
  mapAttachStopReason(event) {
    const rawReason = typeof event.body?.reason === "string" ? event.body.reason : "";
    const rawText = typeof event.body?.text === "string" ? event.body.text : void 0;
  ...
  updateAttachStoppedLocation(event) {
    const sourcePathRaw = event.body?.source;
    const lineRaw = event.body?.line;
    const functionRaw = event.body?.function;
    const sourcePath = typeof sourcePathRaw === "string" && sourcePathRaw.length > 0 ? sourcePathRaw : void 0;
    const line = typeof lineRaw === "number" ? lineRaw : void 0;
    const functionName = typeof functionRaw === "string" && functionRaw.length > 0 ? functionRaw : void 0;
  ...
  updateAttachStoppedState(event) {
  ...
  async flushPendingAttachBreakpoints() {
    ...
    for (const [path, clientLines] of this._pendingAttachBreakpoints) {
      try {
        ...
        this.sendEvent(new import_debugadapter.OutputEvent(`Set breakpoints sync failed for ${path}: ${error.message}
`, "console"));
      }
    }
  }
  getAttachStopForUi() {
    return this._attachStoppedState ?? (this._attachStopHistory.length > 0 ? this._attachStopHistory[0] : void 0);
  }
  buildAttachScopeVariables(scope) {
    ...
    return [
      ...
      new RuntimeVariable("threadId", _FosDebugSession.threadID),
  ...
  disconnectRequest(response, args2, request) {
    ...
    if (this._attachTransport) {
      ...
      this._attachTransport.sendRequest("disconnect").catch(() => void 0);
      this._attachTransport.close();
      this._attachTransport = void 0;
    }
    this._isAttachedTransport = false;
    this._attachStoppedLocation = void 0;
    this._attachStoppedState = void 0;
    this._attachStopHistory = [];
  ...
  async attachRequest(response, args2) {
    ...
    try {
      ...
      this._attachStoppedState = void 0;
      this._attachStopHistory = [];
      this._attachTransport.setCloseHandler((reason) => {
        ...
        this._attachTransport = void 0;
        if (reason) {
          this.sendEvent(new import_debugadapter.OutputEvent(`Attach transport closed: ${reason}
`, "console"));
        }
        this.sendEvent(new import_debugadapter.TerminatedEvent());
      });
      this._attachTransport.setEventHandler((event) => {
        if (event.event === "stopped") {
          this.updateAttachStoppedState(event);
          const mapped = this._attachStoppedState;
          if (!mapped) {
            this.sendEvent(new import_debugadapter.StoppedEvent("pause", _FosDebugSession.threadID));
            return;
          }
          this.sendEvent(new import_debugadapter.StoppedEvent(mapped.reason, _FosDebugSession.threadID, mapped.text));
          return;
        }
        ...
        if (event.event === "resumed") {
          this._attachStoppedState = void 0;
          this.sendEvent(new import_debugadapter.ContinuedEvent(_FosDebugSession.threadID, true));
        }
      });
      ...
      this.sendResponse(response);
      this.sendEvent(new import_debugadapter.OutputEvent(`Attached to ${target.targetName ?? "AngelScript runtime"} (pid=${target.processId}) via ${target.endpoint}
`, "console"));
      if (handshake) {
        this.sendEvent(new import_debugadapter.OutputEvent(`Attach handshake: ${JSON.stringify(handshake)}
`, "console"));
      }
      this.sendEvent(new import_debugadapter.OutputEvent(`Attach capabilities: ${JSON.stringify(capabilities)}
`, "console"));
  ...
  async setBreakPointsRequest(response, args2) {
    ...
    if (this._isAttachedTransport && this._attachTransport) {
      try {
        ...
        response.body = {
          breakpoints: responseBreakpoints.map((bp, index) => {
            const debuggerLine = typeof bp?.line === "number" ? bp.line : debuggerLines[index] ?? 0;
            const verified = bp?.verified !== false;
            const dapBp = new import_debugadapter.Breakpoint(verified, this.convertDebuggerLineToClient(debuggerLine));
            dapBp.id = index + 1;
            return dapBp;
          })
        };
        ...
        this.sendErrorResponse(response, {
          id: 2007,
          format: `Set breakpoints failed: ${error.message}`
        });
      }
      return;
    }
  ...
  threadsRequest(response) {
    if (this._isAttachedTransport) {
      response.body = {
        threads: [new import_debugadapter.Thread(_FosDebugSession.threadID, "attach-thread")]
      };
      this.sendResponse(response);
      return;
    }
    response.body = {
      threads: [
        new import_debugadapter.Thread(_FosDebugSession.threadID, "thread 1"),
        new import_debugadapter.Thread(_FosDebugSession.threadID + 1, "thread 2")
      ]
    };
    this.sendResponse(response);
  }
  async stackTraceRequest(response, args2) {
    if (this._isAttachedTransport) {
      ...
      if (this._attachTransport) {
        try {
          ...
          const stackFrames = rawFrames.map((frame, ix) => {
            const frameId = typeof frame.id === "number" ? frame.id : startFrame2 + ix + 1;
            const frameName2 = typeof frame.name === "string" && frame.name.length > 0 ? frame.name : "attach-frame";
            const source2 = typeof frame.source === "string" && frame.source.length > 0 ? this.createSource(frame.source) : void 0;
            const line2 = typeof frame.line === "number" ? this.convertDebuggerLineToClient(frame.line) : 1;
            const sf = new import_debugadapter.StackFrame(frameId, frameName2, source2, line2);
      ...
      const historyLocation = this._attachStopHistory.length > 0 ? this._attachStopHistory[0].location : void 0;
      const location = this._attachStoppedLocation ?? historyLocation;
      const hasFrame = !!location;
      const source = location?.sourcePath ? this.createSource(location.sourcePath) : void 0;
      ...
      const frames = hasFrame ? [new import_debugadapter.StackFrame(1, frameName, source, line)] : [];
      const sliced = frames.slice(startFrame2, startFrame2 + maxLevels2);
      response.body = {
        stackFrames: sliced,
  ...
  scopesRequest(response, args2) {
    if (this._isAttachedTransport) {
      ...
      response.body = {
        scopes: [
          new import_debugadapter.Scope("Locals", localsRef, false),
          new import_debugadapter.Scope("Globals", globalsRef, true)
        ]
      };
      this.sendResponse(response);
      return;
...
`));
  ...
  createSource(filePath) {
    return new import_debugadapter.Source((0, import_path_browserify.basename)(filePath), this.convertDebuggerPathToClient(filePath), void 0, void 0, "fos-adapter-data");
  }
};
