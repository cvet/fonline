...
if [ "$1" = "linux-client" ]; then
    TARGET=linux
    BUILD_TARGET="-DFONLINE_BUILD_CLIENT=1 -DCMAKE_BUILD_TYPE=Debug"
    ...
    TARGET=linux
    BUILD_TARGET="-DFONLINE_BUILD_SERVER=1 -DCMAKE_BUILD_TYPE=Debug"
elif [ "$1" = "linux-single" ]; then
    TARGET=linux
    BUILD_TARGET="-DFONLINE_BUILD_SINGLE=1 -DCMAKE_BUILD_TYPE=Debug"
    ...
    TARGET=linux
    BUILD_TARGET="-DFONLINE_BUILD_MAPPER=1 -DCMAKE_BUILD_TYPE=Debug"
elif [ "$1" = "linux-ascompiler" ]; then
    TARGET=linux
    BUILD_TARGET="-DFONLINE_BUILD_ASCOMPILER=1 -DCMAKE_BUILD_TYPE=Debug"
elif [ "$1" = "linux-baker" ]; then
    TARGET=linux
    BUILD_TARGET="-DFONLINE_BUILD_BAKER=1 -DCMAKE_BUILD_TYPE=Debug"
elif [ "$1" = "unit-tests" ]; then
    TARGET=linux
    BUILD_TARGET="-DFONLINE_UNIT_TESTS=1 -DCMAKE_BUILD_TYPE=Debug"
elif [ "$1" = "code-coverage" ]; then
    TARGET=linux
    BUILD_TARGET="-DFONLINE_CODE_COVERAGE=1 -DCMAKE_BUILD_TYPE=Debug"
else
