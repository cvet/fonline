...
from package import RESOURCE_PACK_CODEC_DEFLATE, RESOURCE_PACK_HEADER_SIZE, write_resource_pack
...
def measure_pack(job: tuple[str, str, str, int, int]) -> dict:
	pack_name, pack_dir, work_dir, compress_level, min_gain_percent = job
...
def main() -> int:
	...
	with tempfile.TemporaryDirectory(prefix='fores_measure_') as work_dir:
		jobs = [(path.name, str(path), work_dir, args.compress_level, args.min_gain_percent) for path in pack_dirs]
