...
function activate(context) {
  ...
  let disposable =
      vscode.commands.registerCommand('extension.fonlineBuild', function() {
        if (busy) return;
        ...
        var csprojFile =
            files.find(f => path.extname(f).toLowerCase() === '.csproj');
        if (csprojFile == undefined) {
          vscode.window.showInformationMessage(
              'Project file not found (.csproj)');
          return;
        }
        ...
        vscode.commands.executeCommand('workbench.action.files.saveAll')
            .then(
                r => {
                  setTimeout(() => {
                    if (foBuild(csprojPath, 'Server') &&
                        foBuild(csprojPath, 'Client') &&
                        foBuild(csprojPath, 'Mapper'))
                      vscode.window.showInformationMessage(
                          'FOnline build successful');
