...
{
	// A comparison assigned to bool is generated by the Compare node. HLSL implicitly
	// truncates a vector comparison to its first component while WGSL rejects it,
	// so extract .x explicitly to keep the behavior same as DirectX.
	const std::regex comparePattern(
		R"(^\s*((?:vec[234]<f32>|[A-Za-z_]\w*)(?:\([^()]*\))?)\s*(<=|>=|==|!=|<|>)\s*((?:vec[234]<f32>|[A-Za-z_]\w*)(?:\([^()]*\))?)\s*$)");

	std::smatch match;
	if (!std::regex_match(expression, match, comparePattern))
	{
		return expression;
	}

	const auto adaptOperand = [&variableTypes](const std::string& operand) -> std::string
	{
		if (IsVectorType(operand))
		{
			// constructor like vec2<f32>(0.5, 0.5)
			return operand + ".x";
		}
	...
	return adaptOperand(match[1].str()) + " " + match[2].str() + " " + adaptOperand(match[3].str());
}
