...
namespace Effekseer.GUI
{
	...
	public class Commands
	{
		...
		{
			if (!string.Equals(System.IO.Path.GetExtension(Core.Root.GetFullPath()), ".efkproj", StringComparison.OrdinalIgnoreCase) ||
				!System.IO.File.Exists(Core.Root.GetFullPath()))
			{
				return SaveAs();
			}
			else
		...
		public static bool SaveAs()
		{
			...
			if (!string.IsNullOrEmpty(result))
			{
				...
				if (!string.Equals(System.IO.Path.GetExtension(filepath), ".efkproj", StringComparison.OrdinalIgnoreCase))
				{
					filepath += ".efkproj";
				}
