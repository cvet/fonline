...
using PythonHosting = IronPython.Hosting;
using PythonRuntime = IronPython.Runtime;
...
namespace Effekseer.Script
{
	class Compiler
	{
		...
		static ScriptingHosting.ScriptEngine PythonEngine
		{
			get;
			set;
		}
		...
		static public void Initialize()
		{
			...
			PythonEngine = PythonHosting.Python.CreateEngine();
		}
		...
		static public bool CompileScript<T>(string path, out T script, out string error) where T : ScriptBase
		{
			...
			{
				var scope = compiled as ScriptingHosting.ScriptScope;
				...
				try
				{
					...
					if (typeof(T) == typeof(CommandScript))
					{
						var pyfunction = scope.GetVariable("Call") as PythonRuntime.PythonFunction;
						Action function = () =>
						{
