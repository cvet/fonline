...
namespace Effekseer
{
	public class Core
	{
		...
		public static void SaveTo(string path)
		{
			...
			if (!string.Equals(System.IO.Path.GetExtension(fullPath), ".efkproj", StringComparison.OrdinalIgnoreCase))
			{
				throw new System.IO.InvalidDataException("Effekseer authoring projects must be saved as .efkproj XML");
			}

			Root.SetFullPath(fullPath);
			var document = SaveAsXmlDocument(Core.Root);
