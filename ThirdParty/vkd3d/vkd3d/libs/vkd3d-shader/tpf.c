...
/* (FOnline Patch) HLSL codegen never marks a constant buffer dynamically indexed, although the program may
 * index it through a register, so derive the access pattern from the program itself. */
static bool tpf_is_constant_buffer_dynamically_indexed(struct vsir_program *program, unsigned int id)
{
    ...
    for (ins = vsir_program_iterator_head(&it); ins; ins = vsir_program_iterator_next(&it))
    {
        for (i = 0; i < ins->src_count; ++i)
        {
            ...
            if (reg->type != VSIR_REGISTER_CONSTBUFFER || reg->idx[0].offset != id)
                continue;

            for (j = 1; j < reg->idx_count; ++j)
            {
                if (reg->idx[j].rel_addr)
                    return true;
            }
        }
    }
...
static void tpf_dcl_constant_buffer(struct tpf_compiler *tpf, const struct vkd3d_shader_instruction *ins)
{
    ...
    if ((ins->flags & VKD3DSI_INDEXED_DYNAMIC)
            || tpf_is_constant_buffer_dynamically_indexed(tpf->program, cb->src.reg.idx[0].offset)) /* (FOnline Patch) see above. */
