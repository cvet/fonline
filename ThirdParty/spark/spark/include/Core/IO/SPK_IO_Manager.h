...
#include <fstream>
#include <map>
#include <mutex> // (FOnline Patch) Protect custom object registration used by parallel resource bakers.
#include <string>
...
namespace IO
{
	...
	public :
		...
		template<typename T> void ensureObjectRegistered(); // (FOnline Patch) Idempotent thread-safe registration for application extensions.
	...
	private :
		...
		mutable std::mutex registeredObjectsMutex; // (FOnline Patch) Resource packs construct custom bakers concurrently.
	...
	{
		const std::lock_guard<std::mutex> lock(registeredObjectsMutex); // (FOnline Patch)
	...
	template<typename T> void IOManager::ensureObjectRegistered() // (FOnline Patch)
	{
	...
	{
		const std::lock_guard<std::mutex> lock(registeredObjectsMutex); // (FOnline Patch)
		const std::string name = T::asName(); // (FOnline Patch) Use the class-name helper generated by SPK_IMPLEMENT_OBJECT.
	...
	{
		const std::lock_guard<std::mutex> lock(registeredObjectsMutex); // (FOnline Patch)
		return registeredObjects.find(T::asName()) != registeredObjects.end(); // (FOnline Patch) Use the class-name helper generated by SPK_IMPLEMENT_OBJECT.
	}
