...
void Earcut<N>::indexSegment(Node* head, Node* stop) {
    ...
    do {
        ...
        blockHead[b] = p;
        // (FOnline Patch) Scope-specific names avoid MSVC C4458 against the Earcut bbox members.
        double blockMinX = std::numeric_limits<double>::max();
        double blockMinY = std::numeric_limits<double>::max();
        double blockMaxX = std::numeric_limits<double>::lowest();
        double blockMaxY = std::numeric_limits<double>::lowest();
        int32_t k = 0;
        do {
            ...
            if (p->x < blockMinX) blockMinX = p->x;
            if (p->x > blockMaxX) blockMaxX = p->x;
            if (p->y < blockMinY) blockMinY = p->y;
            if (p->y > blockMaxY) blockMaxY = p->y;
            if (c->x < blockMinX) blockMinX = c->x;
            if (c->x > blockMaxX) blockMaxX = c->x;
            if (c->y < blockMinY) blockMinY = c->y;
            if (c->y > blockMaxY) blockMaxY = c->y;
            p = c;
        } while (++k < K && p != stop);
        blockStop[b] = p;
        const std::size_t g = b * 4;
        blockBBox[g] = blockMinX;
        blockBBox[g + 1] = blockMinY;
        blockBBox[g + 2] = blockMaxX;
        blockBBox[g + 3] = blockMaxY;
    } while (p != stop);
}
...
bool Earcut<N>::intersectsPolygon(const Node* a, const Node* b) {
    ...
    // (FOnline Patch) Scope-specific names avoid MSVC C4458 against the Earcut bbox members.
    const double diagonalMinX = std::min<double>(a->x, b->x);
    const double diagonalMaxX = std::max<double>(a->x, b->x);
    const double diagonalMinY = std::min<double>(a->y, b->y);
    const double diagonalMaxY = std::max<double>(a->y, b->y);

    const Node* p = a;
    do {
        const Node* n = p->next;
        if ((p->x > diagonalMaxX && n->x > diagonalMaxX) || (p->x < diagonalMinX && n->x < diagonalMinX) ||
            (p->y > diagonalMaxY && n->y > diagonalMaxY) || (p->y < diagonalMinY && n->y < diagonalMinY)) {
            p = n;
            continue;
        }
