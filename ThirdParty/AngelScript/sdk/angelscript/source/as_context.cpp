...
asCContext::asCContext(asCScriptEngine *engine, bool holdRef)
{
	...
	m_returnValueSize           = 0;
	m_stackIndex                = 0;
	BeginScriptCall             = 0; // (FOnline Patch)
	EndScriptCall               = 0; // (FOnline Patch)
}
...
int asCContext::Execute()
{
	...
	if (m_status != asEXECUTION_SUSPENDED && BeginScriptCall) // (FOnline Patch)
	...
	if (m_status == asEXECUTION_FINISHED && EndScriptCall) // (FOnline Patch)
		EndScriptCall(this);
...
void asCContext::CallScriptFunction(asCScriptFunction *func)
{
	...
	if (BeginScriptCall) // (FOnline Patch)
		BeginScriptCall(this, m_currentFunction);
	...
	INSTRUCTION(asBC_RET):
		{
			...
			if (EndScriptCall) // (FOnline Patch)
				EndScriptCall(this);
