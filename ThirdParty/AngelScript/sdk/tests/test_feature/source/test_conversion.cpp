...
bool Test()
{
	...
	// Test conversion double to uint where value is too big to hold in a 32bit int without changing sign
	// Reported by Aaron Baker
	{
		...
		mod->AddScriptSection("script",
			"void casting_error() \n"
			"{ \n"
			"	double x = 3234567890; \n" // number is too big to fit in a signed 32bit int without changing sign
			"	uint y = x; \n"
			"	assert( y == 3234567890 ); \n"
			"   double x2 = -100; \n"
			"   uint y2 = x2; \n"
			"   assert( y2 == 4294967196 ); \n" // 4294967196 = unsigned int(-100)
			"} \n");
		int r = mod->Build();
		if( r < 0 )
			TEST_FAILED;

		r = ExecuteString(engine, "casting_error();", mod);
		if( r != asEXECUTION_FINISHED )
			TEST_FAILED;

		engine->ShutDownAndRelease();
		if( bout.buffer != "script (1, 1) : Info    : Compiling void casting_error()\n"
						   "script (4, 11) : Warning : Float value truncated in implicit conversion to integer\n"
						   "script (7, 14) : Warning : Float value truncated in implicit conversion to integer\n")
		{
