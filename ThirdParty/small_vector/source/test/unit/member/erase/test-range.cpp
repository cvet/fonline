...
{
    using vector_type = gch::small_vector<T, 7, Allocator>;

  // Make sure erasing an empty range has no effect.
  {
  ...
  {
    vector_type v ({ 1, 2, 3, 4, 5, 6, 7 }, alloc);

    v.erase (v.begin (), std::next (v.begin (), 2));
    CHECK (vector_type { 3, 4, 5, 6, 7 } == v);

    v.erase (v.cbegin (), std::next (v.cbegin (), 2));
    CHECK (vector_type { 5, 6, 7 } == v);
  }
  ...
  // Tests while inlined.
  {
    ...
    pos = v.erase (v.begin (), std::next (v.begin (), 2));
    ...
    pos = v.erase (std::next (v.begin ()), std::next (v.begin (), 3));

    CHECK (vector_type { 3, 6, 7 } == v);
    CHECK (std::next (v.begin ()) == pos);
    CHECK_IF_NOT_CONSTEXPR (v.inlined ());

    // Erase from the end.
    pos = v.erase (std::prev (v.end (), 2), v.end ());

    CHECK (vector_type { 3 } == v);
    CHECK (v.end () == pos);
    CHECK_IF_NOT_CONSTEXPR (v.inlined ());
  }
  ...
  {
    vector_type v ({ 1, 2, 3, 4, 5, 6, 7, 8 }, alloc);
    typename vector_type::iterator pos;
    ...
    CHECK (vector_type { 3, 4, 5, 6, 7, 8 } == v);
    CHECK (v.begin () == pos);
    CHECK (! v.inlined ());
...
test_exceptions (void)
{
  ...
  // Tests while inlined.
  {
    ...
    {
      auto first = std::next (v.begin (), static_cast<std::ptrdiff_t> (pos_idx));
      for (std::size_t l = 1; l < v.size () - pos_idx; ++l)
      {
        auto last = std::next (first, static_cast<std::ptrdiff_t> (l));
        for (std::size_t j = 0; j < v.size () - pos_idx - l; ++j)
        {
          exception_trigger::push (j);
          EXPECT_TEST_EXCEPTION (v.erase (first, last));

          const std::size_t moved_first = pos_idx + j - 1;
          const std::size_t moved_last = moved_first + l;

          for (std::size_t k = pos_idx + l; k < v.size () - 1; ++k)
          {
            if (moved_first <= k && k < moved_last)
