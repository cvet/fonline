...
test_exceptions (void)
{
  ...
  //   No reallocation:
  //     - Construction of the element at the end (No change).     (1)
  //   Reallocation:
  //     - Allocation too large (No change).                       (2)
  //     - Construction of the element at the end (No change).     (3)
  //     - Moving of elements after construction of one at the end (4)

  using vector_type = gch::small_vector<triggering_type, 4, verifying_allocator<triggering_type>>;

  // Throw during construction of the element at the end. (1)
  {
    vector_type v { 1, 2, 3 };
    vector_type v_save = v;

    exception_trigger::push (0);
    EXPECT_TEST_EXCEPTION (v.emplace_back (4));

    CHECK (v == v_save);
  }

  // Throw because of a length error. (2)
  {
    ...
    GCH_TRY
    {
      EXPECT_THROW (w.emplace_back (static_cast<std::int8_t> (2)));
    }
  ...
  // Throw during construction of the element at the end (while reallocating). (3)
  {
    vector_type v { 1, 2, 3, 4 };
    vector_type v_save = v;

    exception_trigger::push (0);
    EXPECT_TEST_EXCEPTION (v.emplace_back (5));
