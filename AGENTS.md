...
- **Never `#include` an STL header anywhere in the project.** The standard library is not included per-file; the whole STL surface in use is included centrally through `BasicCore.h`. If a standard-library facility is missing, add its include to `BasicCore.h`, never to the consuming file.
