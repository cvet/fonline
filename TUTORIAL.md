...
- the platform prerequisites described in [Build Workflow](Docs/BuildWorkflow.md).

The canonical source is [Examples/MinimalProject](Examples/MinimalProject/README.md). Do not use an embedding game's files to complete this tutorial.
...
Use [Getting Started](Docs/GettingStarted.md) for the documentation map, [Embedding FOnline in a Game Project](Docs/EmbeddingProject.md) before turning the scaffold into a separate repository, and [Baking Pipeline](Docs/BakingPipeline.md) before adding resource types. A playable server/client lesson and public project template remain tracked in the [production documentation plan](Docs/ProductionDocumentationPlan.md#phase-4---canonical-starter-and-first-run-tutorials).
