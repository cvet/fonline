...
internal static class BootstrapScenarios
{
    ...
    internal static void RunIsolated(string scenario)
    {
        ...
        using Process process =
            Process.Start(start) ?? throw new InvalidOperationException("Could not start bootstrap scenario");
    ...
    internal static int Run(string scenario)
    {
        ...
        string temporaryDirectory =
            Path.Combine(Path.GetTempPath(), "FOnlineBootstrap_" + Guid.NewGuid().ToString("N"));
        try {
            ...
            try {
                ...
                when (FailStaticConstructor && exception.GetBaseException().Message == "Bootstrap failure sentinel") {
                failed = true;
            }
...
internal static class OwnershipProbe
{
    ...
    public static void Register() =>
        ScriptFuncRegistration.RegisterAttributedScriptFuncs(typeof(BootstrapHandlerAttribute), "BootstrapHandler");
