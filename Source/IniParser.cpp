...
int IniParser::GetInt(const char* app_name, const char* key_name, int def_val)
{
	...
	if(!j) return def_val;
	num[j]=0;
	if(Str::CompareCase(num,"true")) return 1;
	if(Str::CompareCase(num,"false")) return 0;
	return atoi(num);
}
...
label_DefVal:
	ret_buf[0]=0;
	if(def_val) Str::Copy(ret_buf,MAX_FOTEXT,def_val);
	return false;
}
...
char* IniParser::GetApp(const char* app_name)
{
	...
	uint iter=0;
	if(lastAppPos && Str::Compare(app_name,lastApp)) iter=lastAppPos;
	...
	while(!str.eof())
	{
		...
		if(key_end)
		{
			*key_end=0;
			...
			if(Str::Length(line)) cachedKeys.insert(line);
		}
	}
}
