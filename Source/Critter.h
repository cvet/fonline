...
public:
	...
	void SetMaps(uint map_id, ushort map_pid){Data.MapId=map_id;Data.MapPid=map_pid;}
...
public:
	...
	void SetBreakTime(uint ms){breakTime=ms; startBreakTime=Timer::GameTick(); ApRegenerationTick=0;}
	void SetBreakTimeDelta(uint ms){uint dt=(Timer::GameTick()-startBreakTime); if(dt>breakTime) dt-=breakTime; else dt=0; if(dt>ms) dt=0; SetBreakTime(ms-dt);}
	...
	void SetFollowCrId(uint crid){ChangeParam(ST_FOLLOW_CRIT); Data.Params[ST_FOLLOW_CRIT]=crid;}
	...
	void SetHome(uint map_id, ushort hx, ushort hy, uchar dir){Data.HomeMap=map_id; Data.HomeX=hx; Data.HomeY=hy; Data.HomeOri=dir;}
