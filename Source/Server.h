...
public:
	...
	static HoloInfo* GetHoloInfo(uint id){HoloInfoMapIt it=HolodiskInfo.find(id); return it!=HolodiskInfo.end()?(*it).second:NULL;}
...
#define BANS_FNAME_EXPIRED        "expired.txt"
	...
	static ClientBanned* GetBanByIp(uint ip){ClientBannedVecIt it=std::find(Banned.begin(),Banned.end(),ip); return it!=Banned.end()?&(*it):NULL;}
	...
	static ClientData* GetClientData(uint id){ClientDataVecIt it=std::find(ClientsData.begin(),ClientsData.end(),id); return it!=ClientsData.end()?&(*it):NULL;}
