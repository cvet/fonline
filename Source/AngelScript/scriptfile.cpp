...
{
	CScriptFile *file = (CScriptFile*)gen->GetObject();
	CScriptString *f = (CScriptString*)gen->GetArgAddress(0);
	CScriptString *m = (CScriptString*)gen->GetArgAddress(1);
	int r = file->Open(*f, *m);
...
int CScriptFile::Open(const CScriptString &filename, const CScriptString &mode)
{
	...
	std::string mode_ = mode.c_std_str();

    // Validate the mode
#if AS_WRITE_OPS == 1
    if( mode_ != "r" && mode_ != "w" && mode_ != "a" )
#else
	if( mode_ != "r" )
#endif
        return -1;
    ...
    // Open the file
    file = fopen(myFilename.c_str(), mode_.c_str());
    if( file == 0 )
        return -1;
...
int CScriptFile::ReadString(unsigned int length, CScriptString &str)
{
	if( file == 0 )
		return 0;
...
CScriptString* CScriptFile::ReadWord()
{
	if( file == 0 )
		return 0;
...
int CScriptFile::ReadLine(CScriptString &str)
{
	if( file == 0 )
		return 0;
