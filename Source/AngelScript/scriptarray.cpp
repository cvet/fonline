...
void* ScriptArray_First(CScriptArray* array)
{
	return array->At(0);
}

void ScriptArray_RemoveFirst(CScriptArray* array)
{
	array->RemoveAt(0);
}

void* ScriptArray_Last(CScriptArray* array)
{
	return array->At(array->IsEmpty() ? 0 : array->GetSize() - 1);
}

void ScriptArray_Grow(CScriptArray* array, asUINT numElements)
{
	if (numElements == 0)
		return;

	array->Resize(array->GetSize() + numElements);
}

void ScriptArray_InsertFirst(CScriptArray* array, void *value)
{
	array->InsertAt(0, value);
}

void RegisterAddScriptArray(asIScriptEngine *engine)
{
	int r = engine->RegisterObjectMethod("array<T>", "T &first()", asFUNCTION(ScriptArray_First), asCALL_CDECL_OBJFIRST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "const T &first() const", asFUNCTION(ScriptArray_First), asCALL_CDECL_OBJFIRST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "T &last()", asFUNCTION(ScriptArray_Last), asCALL_CDECL_OBJFIRST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "const T &last() const", asFUNCTION(ScriptArray_Last), asCALL_CDECL_OBJFIRST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void insertFirst(const T&in)", asFUNCTION(ScriptArray_InsertFirst), asCALL_CDECL_OBJFIRST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void removeFirst()", asFUNCTION(ScriptArray_RemoveFirst), asCALL_CDECL_OBJFIRST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void grow(uint)", asFUNCTION(ScriptArray_Grow), asCALL_CDECL_OBJFIRST);	assert(r >= 0);	
}
