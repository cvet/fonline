...
TEST_CASE("ModelMeshDataWireHeader")
{
    ...
    SECTION("Rejects wrong magic")
    {
        ...
        magic.front() = byte {'X'};
        vector<byte> data = MakeModelMeshTestData(magic, MODEL_MESH_SCHEMA_VERSION, MODEL_MESH_SUPPORTED_FLAGS);
        DataReader reader {{data.data(), data.size()}};
    ...
    {
        vector<byte> data = MakeModelMeshTestData(MODEL_MESH_MAGIC, uint16_t {MODEL_MESH_SCHEMA_VERSION + 1}, MODEL_MESH_SUPPORTED_FLAGS);
        DataReader reader {{data.data(), data.size()}};
    ...
    {
        vector<byte> data = MakeModelMeshTestData(MODEL_MESH_MAGIC, MODEL_MESH_SCHEMA_VERSION, uint16_t {1});
        DataReader reader {{data.data(), data.size()}};
    ...
    {
        vector<byte> complete_data = MakeModelMeshTestData(MODEL_MESH_MAGIC, MODEL_MESH_SCHEMA_VERSION, MODEL_MESH_SUPPORTED_FLAGS);
