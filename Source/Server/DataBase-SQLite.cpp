...
[[nodiscard]] static auto SqliteAllocHeaderOf(void* payload) noexcept -> nptr<SqliteAllocHeader>
{
    ...
    auto bytes = make_nptr(payload).reinterpret_as<byte>();
...
static auto SqliteMemMalloc(int32_t size) -> void*
{
    ...
    auto block = SafeAlloc::MallocRaw(total).reinterpret_as<byte>();
...
static auto SqliteMemRealloc(void* mem, int32_t size) -> void*
{
    ...
    size_t total = numeric_cast<size_t>(size) + sizeof(SqliteAllocHeader);
    auto moved = SafeAlloc::ReallocRaw(base.void_cast(), total).reinterpret_as<byte>();
...
public:
    ...
    explicit DbSQLite(ptr<DataBaseSettings> db_settings, u8string_view storage_dir, DataBasePanicCallback panic_callback) :
...
protected:
    ...
    void EnsureCollection(hstring collection_name, DataBaseKeyType key_type) override
    {
        ...
        u8string sql = u8strex("CREATE TABLE IF NOT EXISTS {} (key BLOB PRIMARY KEY NOT NULL, value BLOB NOT NULL) WITHOUT ROWID", QuoteIdentifier(collection_name));
    ...
    [[nodiscard]] auto GetAllRecordIds(hstring collection_name) const -> vector<DataBaseKey> override
    {
        ...
        u8string sql = u8strex("SELECT key FROM {}", QuoteIdentifier(collection_name));
    ...
    void InsertRecord(hstring collection_name, const DataBaseKey& id, const AnyData::Document& doc) override
    {
        ...
        u8string sql = u8strex("INSERT INTO {} (key, value) VALUES (?, ?)", QuoteIdentifier(collection_name));
    ...
    void UpdateRecord(hstring collection_name, const DataBaseKey& id, const AnyData::Document& doc) override
    {
        ...
        auto key = MakeSqliteKey(id, GetCollectionKeyType(collection_name));
        u8string sql = u8strex("UPDATE {} SET value = ?2 WHERE key = ?1", QuoteIdentifier(collection_name));
    ...
    void DeleteRecord(hstring collection_name, const DataBaseKey& id) override
    {
        ...
        auto key = MakeSqliteKey(id, GetCollectionKeyType(collection_name));
        u8string sql = u8strex("DELETE FROM {} WHERE key = ?", QuoteIdentifier(collection_name));
    ...
    auto TryReconnect() -> bool override
    {
        ...
        try {
            ...
            Execute(u8"BEGIN IMMEDIATE", hstring());
            Execute(u8"ROLLBACK", hstring());
            return true;
        }
...
private:
    ...
    {
    public:
        Statement(const DbSQLite& db, u8string_view sql, hstring context) FO_TSA_REQUIRES(db._storageLocker)
        {
            FO_STACK_TRACE_ENTRY();

            ptr<sqlite3> db_handle = db.GetHandle();
            nptr<sqlite3_stmt> stmt;
            const_span<char> sql_chars = utf8_to_char_span(sql);
            int32_t prepare = sqlite3_prepare_v2(db_handle.get(), sql_chars.data(), numeric_cast<int32_t>(sql_chars.size()), stmt.get_pp(), nullptr);
        ...
        void BindBlob(int32_t index, const vector<byte>& data)
        {
            FO_STACK_TRACE_ENTRY();
        ...
        void BindBlob(int32_t index, const_span<byte> data)
        {
            FO_STACK_TRACE_ENTRY();
        ...
        [[nodiscard]] auto ColumnBlob(int32_t index) const -> const_span<byte>
        {
            FO_STACK_TRACE_ENTRY();

            // The statement stays mutable for the C API even when read through a const accessor.
            auto stmt = make_ptr(_stmt.get_no_const());
            auto data = cast_from_void<const byte*>(sqlite3_column_blob(stmt.get(), index));
            ...
            return const_span<byte> {data.get(), numeric_cast<size_t>(size)};
        }

    private:
        [[nodiscard]] auto LastError() const -> u8string
        {
            FO_STACK_TRACE_ENTRY();

            auto text = make_nptr(sqlite3_errmsg(make_ptr(_db.get_no_const()).get()));
            return text ? utf8_from_terminated_char_span(const_span<char> {text.get(), std::strlen(text.get()) + 1}) : u8string {"unknown"};
        }
    ...
    void OpenDataBase()
    {
        ...
        u8string db_path = u8strex("{}/Storage.sqlite", _storageDir);
        ptr<const char> db_path_ptr = utf8_to_c_str(db_path.view_nt());
        ...
        if (open != SQLITE_OK) {
            ...
            u8string error = db ? utf8_from_terminated_char_span(const_span<char> {sqlite3_errmsg(db.get()), std::strlen(sqlite3_errmsg(db.get())) + 1}) : u8string {"unknown"};
        ...
        Execute(u8"PRAGMA journal_mode = WAL", hstring());
        ...
        Execute(u8"PRAGMA synchronous = NORMAL", hstring());
        Execute(u8"PRAGMA foreign_keys = ON", hstring());
    }
    ...
    [[nodiscard]] u8string LastError() const FO_TSA_REQUIRES(_storageLocker)
    {
        FO_STACK_TRACE_ENTRY();

        if (!_db) {
            return u8string {u8"database is not open"};
        }
    ...
    void Execute(u8string_view sql, hstring context) const FO_TSA_REQUIRES(_storageLocker)
    {
        FO_STACK_TRACE_ENTRY();
    ...
    void WriteDocument(u8string_view sql, hstring collection_name, const DataBaseKey& id, const vector<byte>& key, const AnyData::Document& doc) FO_TSA_REQUIRES(_storageLocker)
    {
        FO_STACK_TRACE_ENTRY();
        ...
        auto bson_data = make_nptr(bson_get_data(&bson)).reinterpret_as<const byte>();
        ...
        Statement stmt {*this, sql, collection_name};
        stmt.BindBlob(1, key);
        stmt.BindBlob(2, const_span<byte> {bson_data.get(), numeric_cast<size_t>(bson.len)});
    ...
    [[nodiscard]] AnyData::Document GetRecordUnlocked(hstring collection_name, const DataBaseKey& id) const FO_TSA_REQUIRES(_storageLocker)
    {
        ...
        auto key = MakeSqliteKey(id, GetCollectionKeyType(collection_name));
        u8string sql = u8strex("SELECT value FROM {} WHERE key = ?", QuoteIdentifier(collection_name));
        ...
        if (!bson_init_static(&bson, make_nptr(value.data()).reinterpret_as<const uint8_t>().get(), value.size())) {
    ...
    [[nodiscard]] static auto QuoteIdentifier(hstring name) -> u8string
    {
        FO_STACK_TRACE_ENTRY();
        ...
        for (size_t i = 0; i < source_view.size(); i++) {
            if (source_view[i] == u8'"') {
                quoted.append(u8string_view::FromChecked(source_view.substr(segment_begin, i - segment_begin)));
                quoted.append(u8"\"\"");
                segment_begin = i + 1;
            }
        }

        quoted.append(u8string_view::FromChecked(source_view.substr(segment_begin)));
        quoted.append(u8"\"");
        return quoted;
    }

    static auto MakeSqliteKey(const DataBaseKey& key, DataBaseKeyType key_type) -> vector<byte>
    {
        FO_STACK_TRACE_ENTRY();
        ...
        const_span<byte> key_bytes = string_to_byte_span(key_str);
        return vector<byte>(key_bytes.begin(), key_bytes.end());
    }

    static auto ParseSqliteKey(const_span<byte> key_data, DataBaseKeyType key_type) -> DataBaseKey
    {
        FO_STACK_TRACE_ENTRY();
    ...
    static auto FormatSqliteDbKey(const DataBaseKey& key) -> u8string
    {
        FO_STACK_TRACE_ENTRY();

        return std::visit(
            [](const auto& value) -> u8string {
                using T = std::decay_t<decltype(value)>;

                if constexpr (std::is_same_v<T, ident_t>) {
                    return u8strex("{}", value);
                }
                else {
                    return utf8_from_char_span(value);
                }
            },
            key);
    }
