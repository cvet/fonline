...
bool ProtoMap::ReadHeader( FileManager& fm, int version )
{
    ...
    header9.DayTime[ 3 ] = 1380;
    header9.DayColor[ 0 ] = 18;
    header9.DayColor[ 1 ] = 128;
    header9.DayColor[ 2 ] = 103;
    header9.DayColor[ 3 ] = 51;
    header9.DayColor[ 4 ] = 18;
    header9.DayColor[ 5 ] = 128;
    header9.DayColor[ 6 ] = 95;
    header9.DayColor[ 7 ] = 40;
    header9.DayColor[ 8 ] = 53;
    header9.DayColor[ 9 ] = 128;
    header9.DayColor[ 10 ] = 86;
    header9.DayColor[ 11 ] = 29;

    if( version >= 9 )
    {
        fm.SetCurPos( 0 );
        if( !fm.CopyMem( &header9, 8 ) )
            return false;
        fm.SetCurPos( 0 );
        if( !fm.CopyMem( &header9, header9.HeaderSize ) )
            return false;
    }
    else if( version >= 7 )
    {
...
bool ProtoMap::ReadTiles( FileManager& fm, int version )
{
    ...
    for( int tx = 0; tx < Header.MaxHexX / 2; tx++ )
    {
        for( int ty = 0; ty < Header.MaxHexY / 2; ty++ )
        {
            ...
            if( roof )
                Tiles.push_back( Tile( roof, tx * 2, ty * 2, 0, 0, 0, true ) );
        }
    }
    delete[] tiles;
    return true;
...
bool ProtoMap::ReadObjects( FileManager& fm, int version )
{
    if( version < 6 )
    {
        ...
        for( uint k = 0; k < count; k++ )
        {
            ...
            if( obj_v5.MapObjType == MAPOBJ_CRITTER )
                obj_v6.MapObjType = MAP_OBJECT_CRITTER;
            else if( obj_v5.MapObjType == MAPOBJ_WALL )
                obj_v6.MapObjType = MAP_OBJECT_SCENERY;
            else if( obj_v5.MapObjType == MAPOBJ_ITEM )
                obj_v6.MapObjType = MAP_OBJECT_ITEM;
            else if( obj_v5.MapObjType == MAPOBJ_SCENERY )
                obj_v6.MapObjType = MAP_OBJECT_SCENERY;
            else
                continue;
            ...
            if( obj_v6.MapObjType == MAP_OBJECT_CRITTER )
            {
                ...
                for( int i = 0; i < 15; i++ )
                    obj_v6.MCritter.ParamIndex[ i ] = -1;
            ...
            else if( obj_v6.MapObjType == MAP_OBJECT_ITEM )
            {
                ...
                if( obj_v6.ContainerUID )
                    obj_v6.MItem.ItemSlot = obj_v5.ItemSlot;
                for( int i = 0; i < 10; i++ )
                    obj_v6.MItem.Val[ i ] = 0;
                ...
                if( proto->Type == ITEM_TYPE_AMMO )
                    obj_v6.MItem.Count = obj_v5.AMMO.Count;
                else if( proto->Type == ITEM_TYPE_MISC )
                    obj_v6.MItem.Count = obj_v5.MISC.Count;
                else if( proto->Type == ITEM_TYPE_DRUG )
                    obj_v6.MItem.Count = obj_v5.DRUG.Count;
                ...
                else if( proto->Type == ITEM_TYPE_CONTAINER || proto->Type == ITEM_TYPE_DOOR )
                {
                    ...
                    if( obj_v5.CONTAINER.IsOpen )
                        SETFLAG( obj_v6.MItem.LockerCondition, LOCKER_ISOPEN );
                }
            }
            else if( obj_v6.MapObjType == MAP_OBJECT_SCENERY )
            {
                ...
                else if( proto->Type == ITEM_TYPE_GENERIC )
                {
                    ...
                    for( int i = 0; i < obj_v6.MScenery.ParamsCount; i++ )
                        obj_v6.MScenery.Param[ i ] = obj_v5.GENERIC.Param[ i ];
                }
            }
    ...
    else     // Version 6, 7, 8, 9
    {
        ...
        for( uint i = 0; i < count; i++ )
        {
            ...
            Str::Copy( mobj->ScriptName, mobj_v9.ScriptName );
            Str::Copy( mobj->FuncName, mobj_v9.FuncName );
            for( int i = 0; i < 10; i++ )
                mobj->UserData[ i ] = mobj_v9.UserData[ i ];
            mobj->MapObjType = mobj_v9.MapObjType;
            ...
            if( mobj->MapObjType == MAP_OBJECT_CRITTER )
            {
                ...
                for( int i = 0; i < 15; i++ )
                    mobj->MCritter.ParamIndex[ i ] = mobj_v9.MCritter.ParamIndex[ i ];
                for( int i = 0; i < 15; i++ )
                    mobj->MCritter.ParamValue[ i ] = mobj_v9.MCritter.ParamValue[ i ];
            }
            else if( mobj->MapObjType == MAP_OBJECT_ITEM )
            {
                ...
                for( int i = 0; i < 10; i++ )
                    mobj->MItem.Val[ i ] = mobj_v9.MItem.Val[ i ];
            }
            else if( mobj->MapObjType == MAP_OBJECT_SCENERY )
            {
                ...
                for( int i = 0; i < 5; i++ )
                    mobj->MScenery.Param[ i ] = mobj_v9.MScenery.Param[ i ];
...
bool ProtoMap::Refresh()
{
    ...
    else
    {
        ...
        if( version_full == FO_MAP_VERSION_V1 )
            version = -1;      // Deprecated
        else if( version_full == FO_MAP_VERSION_V2 )
            version = -2;      // Deprecated
        else if( version_full == FO_MAP_VERSION_V3 )
            version = -3;      // Deprecated
        else if( version_full == FO_MAP_VERSION_V4 )
            version = 4;
        else if( version_full == FO_MAP_VERSION_V5 )
            version = 5;
        else if( version_full == FO_MAP_VERSION_V6 )
            version = 6;
        else if( version_full == FO_MAP_VERSION_V7 )
            version = 7;
        else if( version_full == FO_MAP_VERSION_V8 )
            version = 8;
