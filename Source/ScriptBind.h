...
//	if(engine->RegisterObjectMethod("Map","Item@+ AddItemFromScenery(uint16 hexX, uint16 hexY, uint16 protoId)",asFUNCTION(BIND_CLASS Map_AddItem),asCALL_CDECL_OBJFIRST)<0) BIND_ERROR;
	...
	if(engine->RegisterObjectMethod("Map","uint CountEntire(uint8 entire) const",asFUNCTION(BIND_CLASS Map_CountEntire),asCALL_CDECL_OBJFIRST)<0) BIND_ERROR;
	if(engine->RegisterObjectMethod("Map","bool GetEntireCoords(uint8 entire, uint skip, uint16& hexX, uint16& hexY) const",asFUNCTION(BIND_CLASS Map_GetEntireCoords),asCALL_CDECL_OBJFIRST)<0) BIND_ERROR;
