...
void FOClient::DrawIface()
{
    ...
    {
        DirtyOffscreenSurfaces.insert(DirtyOffscreenSurfaces.end(), Self->PreDirtyOffscreenSurfaces.begin(),
            Self->PreDirtyOffscreenSurfaces.end());
...
void FOClient::SScriptFunc::Crit_GetNameTextInfo(
...
void FOClient::SScriptFunc::Crit_AddAnimationCallback(
...
string FOClient::SScriptFunc::Global_CustomCall(string command, string separator)
{
    ...
    else if (cmd == "Command" && args.size() >= 2)
    {
        ...
        string buf;
        if (!PackNetCommand(
                str, &Self->Bout, [&buf, &separator](auto s) { buf += s + separator; }, Self->Chosen->Name))
            return "UNKNOWN";
    ...
    else if (cmd == "DrawMiniMap" && args.size() >= 6)
    {
        ...
        if (zoom != Self->LmapZoom || x != Self->LmapWMap[0] || y != Self->LmapWMap[1] || x2 != Self->LmapWMap[2] ||
            y2 != Self->LmapWMap[3])
        {
            Self->LmapZoom = zoom;
            Self->LmapWMap[0] = x;
    ...
    ushort& block_hy)
{
    ...
    Self->HexMngr.TraceBullet(from_hx, from_hy, to_hx, to_hy, dist, angle, nullptr, false, &critters, find_type, &block,
...
CScriptArray* FOClient::SScriptFunc::Global_GetPathHex(
...
uint FOClient::SScriptFunc::Global_GetPathLengthHex(
    ushort from_hx, ushort from_hy, ushort to_hx, ushort to_hy, uint cut)
{
    if (from_hx >= Self->HexMngr.GetWidth() || from_hy >= Self->HexMngr.GetHeight())
        SCRIPT_ERROR_R0("Invalid from hexes args.");
...
void FOClient::SScriptFunc::Global_MapMessage(
...
uint FOClient::SScriptFunc::Global_GetFullSecond(
...
bool FOClient::SScriptFunc::Global_SetEffect(
    int effect_type, int effect_subtype, string effect_name, string effect_defines)
{
// Effect types
...
void FOClient::SScriptFunc::Global_GetTime(ushort& year, ushort& month, ushort& day, ushort& day_of_week, ushort& hour,
...
void FOClient::SScriptFunc::Global_GetTextInfo(
...
void FOClient::SScriptFunc::Global_DrawSpriteSize(
...
void FOClient::SScriptFunc::Global_DrawSpritePattern(
...
void FOClient::SScriptFunc::Global_DrawCritter2d(hash model_name, uint anim1, uint anim2, uchar dir, int l, int t,
...
void FOClient::SScriptFunc::Global_DrawCritter3d(
...
void FOClient::SScriptFunc::Global_ActivateOffscreenSurface(bool force_clear)
{
    ...
    if (std::find(Self->PreDirtyOffscreenSurfaces.begin(), Self->PreDirtyOffscreenSurfaces.end(), rt) ==
        Self->PreDirtyOffscreenSurfaces.end())
...
void FOClient::SScriptFunc::Global_PresentOffscreenSurface(int effect_subtype)
{
    ...
    if (effect_subtype < 0 || effect_subtype >= (int)Self->OffscreenEffects.size() ||
        !Self->OffscreenEffects[effect_subtype])
...
void FOClient::SScriptFunc::Global_PresentOffscreenSurfaceExt2(
