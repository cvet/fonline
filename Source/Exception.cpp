...
static LONG WINAPI TopLevelFilterReadableDump( EXCEPTION_POINTERS* except )
{
	...
	const char* dump_str = except ? "CrashDump" : ManualDumpAppendix.c_str();
	...
	if( f )
	{
		...
		SetCurrentDirectory( Str::FormatBuf( FileManager::GetPath( PT_SERVER_ROOT ) ) );
	#else
		SetCurrentDirectory( Str::FormatBuf( FileManager::GetPath( PT_ROOT ) ) );
	#endif
		...
		mess = Str::FormatBuf( DumpMess, dump_path );
	}
	else
	{
		mess = Str::FormatBuf( "Error while create dump file - Error create file, path '%s', err {%u}.", dump_path, GetLastError( ) );
...
{
	string name = file;
	WriteLog( "Runtime assert: %s in %s (%i)\n", message.c_str(), name.c_str(), line );

#if defined ( FO_WINDOWS ) || defined ( FO_LINUX ) || defined ( FO_MAC )
	// Create dump

#ifdef FONLINE_SERVER
	CreateDump( Str::FormatBuf( "AssertFailed_v%u_%s(%i)", SERVER_VERSION, name.c_str(), line ), message );
	ShowMessage( Str::FormatBuf( "Assert failed!\nVersion: %u\nFile: %s (%i)\n\n%s", SERVER_VERSION, name.c_str( ), line, message.c_str( ) ) );
#elif FONLINE_CLIENT
	CreateDump( Str::FormatBuf( "AssertFailed_v%u_%s(%i)", CLIENT_VERSION, name.c_str( ), line ), message );
	ShowMessage( Str::FormatBuf( "Assert failed!\nVersion: %u\nFile: %s (%i)\n\n%s", CLIENT_VERSION, name.c_str( ), line, message.c_str( ) ) );
#elif FONLINE_MAPPER
	CreateDump( Str::FormatBuf( "AssertFailed_v%u_%s(%i)", MAPPER_VERSION, name.c_str( ), line ), message );
	ShowMessage( Str::FormatBuf( "Assert failed!\nVersion: %u\nFile: %s (%i)\n\n%s", MAPPER_VERSION, name.c_str( ), line, message.c_str( ) ) );
#else
	CreateDump( Str::FormatBuf( "AssertFailed_vUNKNOW_%s(%i)", name.c_str( ), line ), message );
	ShowMessage( Str::FormatBuf( "Assert failed!\nVersion: UNKNOW\nFile: %s (%i)\n\n%s", MAPPER_VERSION, name.c_str( ), line, message.c_str( ) ) );
#endif
	// Show message
#endif
