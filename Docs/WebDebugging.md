...
- **the page dies with `RuntimeError: memory access out of bounds` and a stack of `wasm-function[...]` offsets** -> resolve each offset against the `RelWithDebInfo` wasm, which carries DWARF: `Workspace/emsdk/upstream/emscripten/emsymbolizer -t file -s dwarf LF_Client.wasm 0x<offset>` prints the inlined C++ frames with file and line. A trap in portable C++ that native builds never hit is not necessarily a 32-bit problem: libc++ answers some unspecified behaviour differently from MSVC and libstdc++ - its `multimap::find` lands inside a run of equal keys, which walked `ConfigFile` off the end of the section map (see [Configuration and Data Sources](ConfigurationAndDataSources.md)).
...
- **Native code calls into managed code through `mono_runtime_invoke` only.** The interpreter compiles
  nothing, so a classic thunk or an `UnmanagedCallersOnly` entry has no native code to hand out, and the
  production dispatch uses the runtime invoke here as everywhere. `ManagedScript.InteropProbeOnStart=True` in
