...
Use this guide for architecture, authoring, and validation. Use the generated [role reference](generated/native-extension/roles.md), [hook reference](generated/native-extension/hooks.md), [binding rules](generated/native-extension/bindings.md), and [canonical JSON model](generated/native-extension.json) for exact current declarations.
...
Paths and globs resolve against the embedding project's contribution root. Arguments are role/path pairs; an odd argument count or unknown role is a configure-time error. There is no `EDITOR` role. Editor applications link `BakerLib`, so reusable editor-side authoring support normally belongs in `BAKER`; code that is truly shared by all applications belongs in `COMMON`.
...
| Custom resource bakers and authoring support shared with editor | `BAKER` | Compiles into `BakerLib`; `BAKER` is not a script export target. |
...
Project remote calls remain project-authored script metadata and use the baked catalog described in [RemoteCalls.md](RemoteCalls.md). They are not native-extension symbols.

## Engine hooks

Hooks are optional named C++ entry points. Declare a hook with `///@ EngineHook` and the exact signature from the generated [hook reference](generated/native-extension/hooks.md), in a file registered under its owning role. Codegen sees the declaration and omits that hook's fallback from `GenericCode-Common.gen.cpp`.
...
`AddEngineSources` does not infer dependencies. The embedding project must add libraries, include directories, compile definitions, generated headers, and platform frameworks before core libraries/applications are built. Keep third-party source in its own target and link that target only to the roles that use it.
...
The engine-owned minimal project is the normative small example. A large game project is valuable integration evidence but does not define the reusable contract.

## Updating the engine revision

Treat an Engine gitlink change as an extension compatibility event:

1. compare the old/new [canonical native-extension models](generated/native-extension.json) through [ApiChangeManagement.md](ApiChangeManagement.md);
...
- [EmbeddingProject.md](EmbeddingProject.md) - engine/game repository ownership.
- [BuildToolsPipeline.md](BuildToolsPipeline.md) - stage and library composition.
- [GeneratedApiAndMetadata.md](GeneratedApiAndMetadata.md) - metadata and generated script API.
- [SmartPointers.md](SmartPointers.md) - native pointer vocabulary.
- [Nullability.md](Nullability.md) - native/script nullability boundary.
- [ExceptionSafety.md](ExceptionSafety.md) - lifecycle and mutation exception contracts.
- [ThirdPartyMaintenance.md](ThirdPartyMaintenance.md) - engine-vendored dependency policy; project-vendored dependencies remain project-owned.
