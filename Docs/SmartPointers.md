...
If a borrowed `ptr<T>` **value** is genuinely needed in a deduced local after the check, `.as_ptr()` remains the local spelling. Use it for strict owners/borrows and for already-checked nullable values that must exist as an actual `ptr<T>` variable, not as the place where null is first detected.
...
Owning wrappers borrow the same way: `refcount_ptr<T>` / `refcount_nptr<T>`, `unique_ptr<T>` / `unique_nptr<T>`, `unique_del_ptr<T>` / `unique_del_nptr<T>`, `unique_arr_ptr<T>`, and `shared_ptr<T>` all flow implicitly to `ptr<T>` / `nptr<T>` borrow sites through `get()`. The `ptr<T>` conversion asserts non-null at the conversion point; the `nptr<T>` conversion preserves absence. These conversions never transfer ownership. At call/return/member sites with a known destination type, pass the owner directly. For a strict owner local use `auto owner_borrow = owner.as_ptr();`; for a nullable-owner local that must be present use `auto maybe_owner_borrow = owner.as_nptr();` followed by `FO_VERIFY_AND_THROW(maybe_owner_borrow, "...")` (or `FO_STRONG_ASSERT` inside `noexcept`) before deref. If a nullable owner was just assigned from a factory with a reviewed non-null result and only a few member accesses follow, work with the source owner directly instead of creating a temporary borrowed local and re-checking it.
...
7. Let checked `nptr<T>` values flow directly to `ptr<T>` sites; use an explicit `ptr<T>` local only when a named non-null borrow value is genuinely needed.
