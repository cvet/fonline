...
Array and custom-deleter owners also accept enum element types so the standard `byte` (`std::byte`) raw-buffer domain can use the same ownership vocabulary as other typed buffers. This does not make arbitrary enums numeric data: it exists specifically so `unique_arr_ptr<byte>` and `unique_del_nptr<const byte>` can represent allocated resource bytes without reverting to `uint8_t`.
...
Process/runtime command-line arguments enter through platform or DLL ABI `argc` / `argv` signatures, then immediately move into strict owned text. `CommandLineArgs` owns `u8string` values and exposes `CommandLineArg` / `u8string_view` borrows; it never retains raw argument pointers. Build mutable character copies and a temporary `char**` array only at a final ABI handoff such as the client-runtime export call.
...
For reinterpreting a mutable **byte span** as a typed value span, use the shared `bytes_to_objects<T>(span<byte>)` helper (`CommonHelpers.h`) — it reinterprets the whole byte span (asserting the size is a whole multiple of `sizeof(T)`) into a `span<T>` through `reinterpret_as`; take a subrange with `span::first`/`subspan` on the input or result when fewer elements are wanted. The inverse `object_to_bytes<T>(T&)` exposes one object as its `span<byte>`.

To build a typed **element span** from a borrowed pointer and a length, use `make_span(ptr<T>, length) -> span<T>` (`CommonHelpers.h`) instead of constructing `span<T>{p.get(), length}` — it keeps the `.get()` unwrap inside the helper, and `ptr<const T>` naturally produces `span<const T>`. Raw storage measured in bytes crosses through the explicitly named `make_byte_span(...)` overloads and returns `span<byte>` or `const_span<byte>`. For a `ptr<char>` / `ptr<const char>` holding text, `as_str(size_t len) -> string_view` builds the view directly rather than `string_view{p.get(), len}`.
