...
The slot's `TransparentEggTarget` selects which eligible sprites it cuts. `AnyOccluder` fades all of them for the critter's own egg. `Structure` fades only wall pieces (`IsWall`) and roof tiles, marked on the sprite by `MapSprite::SetEggStructure`, so a prop in front of a wall stays drawn. Script-drawn sprites with a prototype use its wall and roof flags for the same classification; a prototype-marked roof tile also gets the `Always` appearance. `IsCutByTransparentEgg(target, egg_hex, appearence, egg_structure, hex)` in `SpriteManager.h` defines the common rule.
