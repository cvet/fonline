...
The AngelScript `Entity` type has no single native counterpart. `get_entity_from_target` in `BuildTools/codegen.py` promotes every argument, array element and return that spells a script `Entity` to the entity class of the target: `ServerEntity*` on the server, `ClientEntity*` on client and mapper, and the base `Entity*` elsewhere. Entity methods are registered on the concrete script entity types for which they are available, so a prototype cannot reach a server-only method through an abstract `Entity` receiver.
