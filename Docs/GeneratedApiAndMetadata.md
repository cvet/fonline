...
Unsupported metadata shapes that can be registered, such as mutable ref handles, callback return values, and value structs that exceed the fixed buffer rules, remain visible as pending ABI entries instead of moving into a backend-specific declaration list. Property `dict<RefType,V>` and `dict<RefType,V[]>` declarations are rejected earlier by `PropertyRegistrar`: the shared property stream and AngelScript property helpers do not define a fixed size, persistence model, or borrowed-handle policy for ref-type keys, so no property import entry is created for those invalid declarations.
