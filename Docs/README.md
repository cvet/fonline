...
- [Persistence.md](Persistence.md) — server database facade, backend implementations, commit queue, backend-consistent snapshots, and recovery logs.
...
- [ClientMultithreading.md](ClientMultithreading.md) — the runtime `Client.Multithreading` switch and the rule that chooses the worker count: the work scheduler, the two-phase sprite update, model pose phases, ownership boundaries, platform support, and the measurement contract.
...
- [Debugging.md](Debugging.md) — native debugger support, profiling zones and their categories, stack traces, Visual Studio helpers, and client host/runtime validation.
