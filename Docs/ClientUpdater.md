...
Mono and the native interop shims are linked into each managed application. The managed class-library payload, including `System.Private.CoreLib.dll`, is data: the Managed baker writes it under `ManagedRuntime/` in the managed resource pack, and normal resource packaging delivers it with the game assemblies. Only CoreLib and the class libraries the pack's assemblies reach by reference are written ([BakingPipeline.md](BakingPipeline.md#managed-runtime-payload-selection)). Last Frontier assigns that baker to `Scripts`, so the payload lives in `Scripts.zip`; it is not an installation-level companion directory. CoreLib is nevertheless target-platform-specific: its Windows build imports Windows interop, while Unix, Android, and browser builds select their respective implementations.
