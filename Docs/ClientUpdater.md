...
The ABI keeps its existing `int32_t argc, char** argv` shape, so this UTF-8 ownership cutover does not bump `FO_CLIENT_RUNTIME_HOST_ABI_VERSION`. Internally the host owns command-line arguments as strict `u8string` values, creates fresh mutable UTF-8 character copies only for `ClientRuntimeExports::Run`, and destroys them after the call. The runtime immediately validates and copies the received terminated strings back into its own `CommandLineArgs`; it never retains the ABI pointers.
...
Client-side, the `Updater` writes each portion to a `~<filename>` temp file, hashes via streamed `fs_hash_file` ([../Source/Essentials/DiskFileSystem.cpp](../Source/Essentials/DiskFileSystem.cpp)) once complete, then atomically renames over the live file (`ReplaceFileSafely`). The updater hash is FNV-1a 64-bit (separate from the engine's wyhash-backed `hashing_ex::hash`, which is reserved for hash-tables and `hstring`); streaming a chunked file produces the same digest as `fs_hash_bytes` over the full buffer, so server in-memory hashing and client streaming hashing agree by construction. Streaming the hash means even multi-GB resource packs never get fully buffered in RAM on either side.

Updater filesystem ownership is strict UTF-8: the client binary/resource root, staging path, final path, backup path, and server-side `UpdateFileData::DiskPath` are `u8string` values, and disk operations borrow `u8string_view`. The stable client-runtime host ABI still carries `char*` path fields, so the host validates those bounded strings into strict owners immediately before existence/rename/load operations and retains legacy views only for the current diagnostics and ABI handoff.

To avoid rehashing existing packs on every startup (the hashing cost dominates the updater's "is this file already current?" pass for multi-GB resource packs), the disk-side hash check goes through `Updater::IsDiskFileHashMatch`, which caches the result in `CacheStorage` ([Settings.CacheResources](../../LastFrontier.fomain)) under the ASCII key `<full-path-hash>.hash`. The cached entry stores `(size, mtime, hash)`; the cache lookup is invalidated automatically when either size or mtime changes, so a refreshed pack is always rehashed exactly once. Deleting that hash file from the cache directory transparently triggers re-hashing on the next updater pass. Encoding the full physical path into the key hash keeps different same-named files independent without leaking Unicode or platform separators into the logical cache-entry namespace.
...
auto GetUpdateDescriptor(string_view binary_target_name) const -> const_span<byte>;
```
...
- Entries are stored as `UpdateFileData { InMemory, MemoryData?, DiskPath?, Size, Hash }`; descriptors, in-memory file data, and disk-read portions use `byte` buffers. Memory mode keeps the whole pack in RAM for the lifetime of the server. Disk mode keeps only `DiskPath`, `Size`, and the streamed `Hash`; portions are read on demand.
...
- **`*` → per-OS user data dir** (`Platform::GetUserDataBase()` returns validated UTF-8 via env, no SDL/shell32 dependency): Windows wide `%LOCALAPPDATA%`/`%APPDATA%`, macOS `~/Library/Application Support`, Linux `$XDG_DATA_HOME` or `~/.local/share`, then `/<Common.GameName>`.
