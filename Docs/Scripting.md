...
For entity instance methods, the AngelScript dispatch layer validates the receiver before entering the native method body. `Entity_MethodCall` calls `CheckScriptEntityAccessAndNonDestroyed`, which checks server sync coverage and destroyed state for the `self` entity. Do not duplicate that receiver check with `ValidateEntityAccess(self)` inside exported method bodies; add explicit validation only for additional entities reached by the native method or after native code re-enters scripts and then continues operating on entities.
