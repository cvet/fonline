...
At runtime, all managed backends share one Mono VM and root domain, but each `ManagedScriptBackend` owns a collectible `System.Runtime.Loader.AssemblyLoadContext`. `FOnline.ManagedHost.dll` is the only engine bootstrap loaded into the default context; it loads the current target's entry and helper assemblies into the backend context. Consequently C# static fields, type initializers, module state, event-wrapper dictionaries, and dependency identity are isolated per engine instance even when several server/client/mapper engines coexist in one process. Managed objects and delegates must not cross backend boundaries; generated entity wrappers retain the backend identity and reject such use. Backend shutdown first removes native callback roots and managed function descriptors, then asks the host to unload the context and releases its scope GC handle. Mono VM shutdown remains process-owned.
