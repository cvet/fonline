...
Managed property internal calls complete native C++ unwinding before returning through Mono. `CoreScripts/Native.cs` converts an error payload to `InvalidOperationException`, so generated typed properties and generic property-index helpers remain catchable in C# while preserving server entity-cover validation.
