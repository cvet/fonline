...
2. **Backend implementation** — `Source/Scripting/AngelScript/` provides the legacy AngelScript backend;
   ...
   bridges described later in this document; `Source/Scripting/Native/` implements the native C++20-module
   backend (`NativeScriptCore.h`, `NativeScriptBackend`, `NativeScripting`), whose generated per-role module
   API is synthesized by `Source/Tools/NativeScriptSynth.cpp`. Embedding
   projects select the backends through their build configuration, and more than one may be enabled at once.
...
`Source/Scripting/Native/` implements the native C++20-module backend: `NativeScriptCore.h` holds the target-neutral bridge (entity wrappers, event proxies, script-function factories, remote-call codecs and the `ModuleInitContextBase` that engine startup hands to a dispatcher), `NativeScriptBackend` registers under `ScriptSystemBackend::NATIVE_BACKEND_INDEX`, and `InitNativeScripting` wires a role's dispatcher in. The generated surface — one `NativeApi.<Role>` module interface per role plus the per-role dispatchers — is synthesized by `Source/Tools/NativeScriptSynth.cpp`, driven by the standalone `<FO_DEV_NAME>_NativeScriptSynth` app; user modules live in the embedding project's `FO_NATIVE_SCRIPTS_DIR` tree. Engine-side coverage is `Source/Tests/Test_NativeScriptSynth.cpp` for the module-authoring contract (role/folder agreement, required API import, exported initializer, comment stripping, per-role dispatcher coverage). The backend is gated by `FO_NATIVE_SCRIPTING` and needs CMake 3.28+ with a generator CMake can scan C++20 modules with — it is not a drop-in equivalent of the AngelScript or Managed runtimes, and an embedding project may well ship it disabled. The Managed backend is implemented (see above), with engine-side coverage concentrated in `Source/Tests/Test_ManagedScriptBaker.cpp` for project/resource generation, host/helper and runtime payload packaging, cache restoration and repair, incremental tracking, path handling, and stale generated artifact cleanup; embedding projects should still carry end-to-end managed runtime suites for their live gameplay modules.
