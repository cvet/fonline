...
property storage, synchronization contention and callbacks retain their own costs. Non-nullable blittable
structs (`mpos`, `ipos`, `ucolor`, `ident`, and other layouts whose fields are primitives/enums with matching
C++/C# sequential size) use the same unboxed property bridge and the same caller-owned method/event frame.
`hstring` is the interned native entry pointer (`IntPtr Value`, sequential size 8), not a stored hash: method and
event frames memcpy it and a zero handle is the empty `hstring`. Property storage
still keeps the hash and resolves to that handle on get. Collections, nullable proto/fixed-type values, and
layouts with managed references or trailing padding stay on the converting bridge. Scalar, enum, and blittable-struct methods, including
