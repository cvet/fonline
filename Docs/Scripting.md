...
No C++ exception ever leaves an internal call. Mono's managed frames carry no C++ handler, so an exception unwound through them skips every managed `catch` and `finally` on the way - the script cannot catch it, and the runtime's own per-thread state is left half unwound. A failure in a pumped continuation then leaves a nested script entry registered, and the outer entry stops the process on `Managed script entries must unwind in nesting order`. The rule is held by construction: `RegisterInternalCalls` registers every internal call through `AddInternalCall`, which accepts only a `noexcept` function pointer. An internal call that can fail runs its body under `CaptureNativeError` and hands the failure back as an error string - the return value of a call that has none, a trailing `out string? error` of one that has - which the managed wrapper in `CoreScripts/Native.cs` throws as `NativeCallException` (an `InvalidOperationException`) once Mono has left the native frame. A call that cannot fail (entity reference counting, direction math, the diagnostic counters of the calling thread) is `noexcept` as it stands, so a throw that appears there later ends the process deterministically instead of corrupting it.

Managed property internal calls follow the same contract, so generated typed properties and generic property-index helpers remain catchable in C# while preserving server entity-cover validation. Every typed and generic property read takes the entity's shared property auto-lock, and every write takes its exclusive property auto-lock for the complete native access. This matches AngelScript property dispatch and serializes `Game` singleton access against engine-owned frame-property updates; an explicit singleton lock, taken with `using GameLock scope = GameLock.Acquire();`, remains recursive so scripts can make a read-modify-write sequence atomic.
...
native payload and then take the script write path, `Properties::SetValue` - the same one the AngelScript
bridge uses - on every managed bridge (unboxed scalar, fixed-value list and converting): the value is
validated and clamped, a write that leaves the stored bytes unchanged ends there, and only a changing write
runs setters and post-setters, which is where persistence and client sync live. `SetValueFromData` is the
unconditional apply for a value received over the network and is never used for a script write; a script
bridge that took it would save and resend every repeated assignment and fire client reactions for a value
that did not change. Narrow entity accessors keep the integer bridge's unbound-getter default.
...
Managed `ident` has a CoreScript string constructor and `ToString()` override, so identifiers kept as plain text, such as GUI parameters, round-trip without generated value-struct special cases; an `any` holding an identifier converts through the generated operators instead.

### The `any` type

The engine `any` (`any_t` in `Source/Common/Common.h`) holds a value as text, and that text is what properties store, the wire carries and script functions receive. Managed code sees it as the value type `FOnline.any` (`CoreScripts/Any.cs`), never as `object` or `string`, and it converts by the rules the AngelScript backend registers for its `any` (`AngelScriptTypes.cpp`, `Any_*` and `GenericType_AnyConv*`):

- Converting into `any` is implicit and writes the engine's text form: `true`/`false`, integers and floats in the invariant culture (floats in their shortest round-trip form), a `string` as it is, an `hstring` as its text, and an enum as `EnumType::Member`; an enum value with no member fails. A value type is written as its primitive fields in declaration order, nested value types flattened and separated by a space, with an enum field as its number and an `hstring` field as its text. `ManagedScriptBaker` generates these operators on each value struct (`AppendAnyConversions`).
- Converting out of `any` is explicit, and text that does not hold the requested type is an invariant failure rather than a zero. Empty text, the value of `default(any)`, reads as zero, `false`, the empty string and the empty `hstring`. An integer accepts decimal, `0x` hexadecimal, a fraction (truncated) and `true`/`false`, and `EnumType::Member` resolves to the member's value; the target range is checked, and the unsigned 64-bit range ends where the signed one does because the engine reads every integer through `int64`. A float rejects a non-finite value. `ToEnum<T>()` takes `EnumType::Member`, the bare member name or the member's number, since property data reads back into `any` as the number, and rejects anything that names no member. A value type splits its text back into fields: text written by the conversion keeps an empty field in its place, and text typed by hand may repeat spaces.
- Equality compares the text ordinally, and `IsEmpty` tells empty text.
- Integer and boolean reads also accept a numeric literal with the engine's lowercase `f` suffix (`2.9f` becomes integer `2`). Numeric enum reads validate the enum's underlying integer range before constructing it; an overflowing number cannot wrap into a declared member.
- `any` does not implement `IConvertible`, so `System.Convert.ToInt32(arg)` still compiles, because the struct boxes into `object`, and throws `InvalidCastException` when it runs. Convert through the explicit operators.

The managed `any` struct changes the boxed script ABI even though native metadata still names the same `any` type. Engine binaries and baked managed assemblies must be updated together; the compatibility marker in `Common.h` includes this transition.

The bridge boxes `any` as its one text reference and unboxes it the same way (`CreateAnyObject` / `ExtractNativeAnyText` in `ManagedScriptBackend.cpp`, with the managed size checked). Arrays and dictionaries of `any` are `List<any>` and `Dictionary<K, any>`, and the generated `GetAsAny` / `SetAsAny` read and write a plain-data property through it; setting an enum property from `any` takes the member's number, its name or `EnumType::Member` (`Properties::SetPlainDataValueAsAny`), and the AngelScript `any` reads an enum back from the member's number as well. `ScriptFuncRegistration.EngineTypeName` maps `any` to metadata `any` and rejects `object`, which names no engine type. Dispatch by name (`ScriptFunc.Invoke`) converts an argument into an `any` parameter by the implicit rules and an `any` argument into a typed parameter by the explicit ones. Two differences from the AngelScript backend are deliberate: text that is not a float or a boolean fails there as it fails for integers, where AngelScript read it as zero, and a strong type such as `timespan` is written as its raw field, which reads back, where AngelScript formatted it for display.
