...
to an entity by holding that lock or a pinned ancestor on its parent / widen chain. `IsEntityAccessValid()`
is that read-path check; mutating an entity's parentage is stricter and requires its own lock directly.

Parent and Player/Critter widen links publish raw atomic pointers for covered identity checks, while a small
per-link atomic lock serializes the lifetime-sensitive read-plus-`TryAddRef` operation with pointer replacement
and release. The lock cannot fail on an OS resource and is therefore safe inside the existing `noexcept`
accessors. `GetParentRaw()`, `GetPlayerForSend()`, and `GetSyncWidenEntity()` return owning handles
without a load-to-`TryAddRef` use-after-free window. `SyncContext::SyncEntities()` also computes widen closure in two phases: it first acquires
the requested cover, reads and pins links only from covered sources, then retries with the exact closure. If a
reparent or link change races acquisition, verification releases the stale set and recomputes it within the
bounded retry budget. `Source/Tests/Test_ServerEngine.cpp` exercises both symmetric Player/Critter widening and
concurrent parent churn (`ServerEngineSyncContextWidenAndAncestorCover` and
