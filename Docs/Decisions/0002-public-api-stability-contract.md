...
Reachability is not the same as a compatibility promise. The historical `PUBLIC_API.md` mixed current, planned, and obsolete statements and could not be regenerated or checked against source. Freezing every reachable symbol would also prevent necessary engine refactoring.
...
2. A generated canonical API model will normalize those sources into stable symbol IDs and machine-readable JSON.
3. Human reference pages are generated from that model and supplemented with task-oriented examples and explanations.
4. Manual prose must not maintain declaration counts or signatures that can be generated. `Docs/generated/source-inventory.json` is the first deterministic inventory and will be expanded into the full API model.
...
Native-codegen symbols use a separate `///@ ApiContract <id-or-family> <label> ...` metadata tag. The selector resolves against the canonical generated `id` first, then against `family_id` so one reviewed declaration can cover an overload family. The tag is parsed and validated by `BuildTools/codegen.py` but is documentation-only and excluded from the runtime compatibility hash.

Unannotated symbols remain `internal (default)`. An explicit `internal` tag records a reviewed decision without promoting the symbol. `stable`/`experimental` require `Since`; `deprecated` requires deprecation version, live replacement, and removal target. The canonical generator rejects unknown or overlapping selectors and invalid lifecycle/example data.

Contract tags are not a shortcut around owner review. The initial implementation explicitly classifies only the development-only `Game.BreakIntoDebugger` helper as internal; no non-internal promise is inferred from this mechanism.
...
`BuildTools/docs_contract_diff.py` compares the native API, CMake, main BuildTools CLI, package, helper CLI, and native-extension models with the same paths at the pull request base SHA or pre-push revision. The specialized native layer in `BuildTools/docs_api_diff.py` retains symbol/overload semantics; the other models match their source-owned stable IDs. All domains classify additions, documentation, policy, and breaking changes against baseline stability so a simultaneous downgrade to `internal` cannot bypass review.
...
- Focused tests prove that `ApiContract` family selectors cover overloads, deprecated replacements resolve, invalid selectors fail, and docs-only annotations do not change the compatibility hash.
- `BuildTools/docs_contract_diff.py --enforce`, the specialized API comparator, and their focused tests reject undisposed baseline-public removals, shape changes, stability withdrawals, stale digests, and model/parser contract changes across the six generated domains.
- Explicit non-internal classifications and non-codegen contract domains remain required before replacing the `PUBLIC_API.md` route.
...
- [ApiChangeManagement.md](../ApiChangeManagement.md)
