...
1. English (`en`) is canonical. The current flat English files remain the source until the planned tree migration.
2. Russian (`ru`) is a planned whole-document mirror.
3. Human pages moving under `Docs/en` derive their Russian path by replacing `Docs/en/` with `Docs/ru/`.
4. Repository and subsystem README entry points use explicit paired paths such as `README.md` and `README.ru.md`.
5. Stable document IDs, not translated titles, join English and Russian pages.
6. Translation freshness uses a normalized SHA-256 hash of canonical English content. The hash/parity release gate is implemented with the Russian tree, not simulated before translations exist.
...
6. Current source-path URLs remain canonical until the corresponding move lands. Planned URLs in the route catalog are reservations, not live-page claims.
7. Navigation, search, AI delivery, and the rendered version indicator consume the same manifest policy. They may not maintain independent locale or version declarations.
...
- Five README-style entry points need explicit locale pairs because they do not live under `Docs/en`.
- Full bilingual navigation, translation hashes, and language switching remain incomplete until the actual locale trees exist.
