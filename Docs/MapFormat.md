...
Use the generated [map format reference](generated/map-format/index.md) for an exhaustive, revision-pinned contract:

- [syntax and directives](generated/map-format/syntax.md);
- [Map, Critter, and Item properties](generated/map-format/properties.md);
- [ownership, baking, and runtime loading](generated/map-format/baking.md);
- [validation rules with stable IDs](generated/map-format/validation.md);
...
The required first section is `[ProtoMap]`. The only placement sections are `[Critter]` and `[Item]`. Current loaders reject data before `[ProtoMap]`, duplicate `[ProtoMap]` sections, and every other section name. Legacy `[Header]`, `[Tiles]`, and `[Objects]` forms are not part of the current format.
...
`$Name` in `[ProtoMap]` selects the Map prototype ID and the basename of both baked resources:
...
When `$Name` is omitted, the source filename without `.fomap` is used. Keep the source basename and `$Name` equal in normal authored content. A mismatch is legal and useful for a deliberate canonical rename, but it makes targeted baking and source lookup less obvious.
...
Every `[Critter]` and `[Item]` requires `$Proto`:
...
Mapper save is deterministic normalization, not byte-preserving serialization. It:

- writes `[ProtoMap]` first;
- serializes the mapper's current full Map property state;
- preserves `$Text*` extra fields;
- omits `$Name` and `$Parent` control directives;
...
Before using the mapper on hand-authored or generated maps, commit or otherwise preserve a reviewable source snapshot. After saving, inspect the complete diff, restore intentional `$Name` if it differs from the filename, and rebake both outputs.
...
1. parse the file and require exactly one first `[ProtoMap]` plus only `[Critter]` and `[Item]` sections;
2. require explicit positive IDs unique across Critter and Item placements;
3. resolve every `$Proto`, ownership value, `CritterId`, and `ContainerId`;
4. validate receiver properties, side availability, resources, and map bounds;
5. bake both server and client map resources;
6. load representative maps in the server and client or mapper;
7. run project-specific checks for quests, spawns, collision, visual kits, scripts, and gameplay routes.
...
- Keep filename, `$Name`, and project catalog ID aligned.
- Put `[ProtoMap]` first and use no legacy or private section names.
