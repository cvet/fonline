...
- **Remote-call structural limits.** A `///@ RemoteCall` may declare `MaxBytes N` and `MaxCollectionSize N`. Metadata carries both limits for every call: a declaration without either option is baked with the explicit `Limits 0 0` trailer, and registration rejects records without it. The server resolves the call name before allocating its body; the native validator and AngelScript decoder enforce the collection limit before reserve or container creation, including nested dictionary arrays. Unknown calls are rejected before body allocation. Adding the mandatory trailer changed compatibility metadata and therefore requires the corresponding compatibility-version bump.
