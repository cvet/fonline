...
locale: en
permalink: /Docs/en/explanation/content-pipeline/baking.html
---

# Baking Pipeline

This document explains the engine resource baking pipeline: where it is wired, which source files own baker behavior, and how to validate changes. For the broader engine tool map, see [Tools](../../../Tools.md).
...
The baking pipeline is a revision-pinned `experimental` contract. The Engine owns the resource-pack schema, built-in baker names and ordering, incremental/full-rebuild behavior, private runtime payloads, report schema, and the extension hook. An embedding project owns its pack names, input layout, custom bakers, side assignment, content policy, generated project sources, and release acceptance.
...
- give each pack one clear responsibility and a stable name, such as metadata, scripts, text, prototypes, maps, visual resources, or sounds;
- list only the bakers that can transform that pack, instead of applying every baker to one broad input tree;
- place producer packs before consumers and use distinct baker orders for hard dependencies; bakers at the same order may run concurrently and must not consume one another's outputs;
- use `IncludePatterns` and `ExcludePatterns` to keep templates, editor-only inputs, and intentionally private sources out of runtime output;
- use one side-only flag when a pack truly belongs only to server, client, or mapper; an unflagged pack is delivered to server and client;
- prefer authored directories for normal source and `InputFiles` for mounted archives or data packs; do not edit baked output as source;
- run a force bake after an Engine revision, baker contract, global metadata, or dependency-graph change that invalidates more than timestamp-based incremental detection can prove.

For the exact project-configuration grammar and precedence rules, see [Configure a Game Project](../../how-to/build/project-configuration.md). For generated-source ownership and recovery order, see [Generated Content Workflow](../../how-to/build/generated-content.md).

## Built-in baker types

`Source/Tools/Baker.cpp` registers built-in bakers when requested and enabled:

| Config name | Class | Order | Availability |
| --- | --- | ---: | --- |
...
| `Particle` | `ParticleBaker` | 5 | always; selected backend still depends on build options |
| `ProtoText` | `ProtoTextBaker` | 6 | always |
| `ModelInfo` | `ModelInfoBaker` | 6 | `FO_ENABLE_3D` |
| `Proto` | `ProtoBaker` | 7 | always |
| `Map` | `MapBaker` | 8 | always |
...
For `ProtoBaker` section resolution, identity, inheritance, side-specific property applicability, strict values, references, and migrations, use [Prototype Format](../../how-to/content/prototype-format.md) and the generated [prototype-format reference](../../reference/prototype-format/index.md). Concrete project extensions, entity metadata, IDs, and semantic field combinations remain project-owned.

For `.fomap` section order, placement IDs, ownership references, static/dynamic handling, mapper round-trip, and the coupled server/client payloads produced by `MapBaker`, use [Map Format](../../how-to/content/map-format.md) and the generated [map-format reference](../../reference/map-format/index.md). Project map catalogs, composition rules, quests, encounters, and gameplay validation remain project-owned.

`ModelMeshBaker` imports current `.fbx` / `.obj` inputs before `ModelInfoBaker` validates and emits concrete `.fo3d` descriptions. For lexical syntax, includes, layers, attachments, transforms, materials, cuts, runtime composition, and validation, use [Model Format](../../how-to/content/model-format.md) and its generated reference. For animation tuples, `AnimSpeed`, one-step `StateAnimEqual` / `ActionAnimEqual` semantics, `ModelAnimationInfo.foinfo`, runtime distribution, and missing-data behavior, use [Model Animation](../../how-to/content/model-animation.md). Baked mesh/description/config layouts are private baker/runtime contracts and must not be parsed by embedding-project scripts.

`TextBaker` parses `<TextPack>.<Language>.fotxt`, completes changed pack language sets, and normalizes them against the first `Baking.BakeLanguages` entry. `ProtoTextBaker` merges inherited `$Text` fields and emits the five Engine-generated prototype text packs. Use [Text and Localization](../../how-to/content/text-and-localization.md) and the generated [text-format reference](../../reference/text-format/index.md) for exact syntax, fallback, runtime lookup, color-tag, and project-formatting boundaries.

`ImageBaker` imports PNG/TGA and ten legacy/descriptor extensions, composes FOFRM sequences/directions/options, and writes the private RGBA sprite container consumed by the stock client. Use [Image And Sprite Formats](../../how-to/content/image-format.md) and its generated [image-format reference](../../reference/image-format/index.md) for exact source support, FOFRM grammar, legacy selectors, output naming, runtime factory coverage, atlas/cache behavior, and validation. Signed per-frame `NextX` / `NextY` then enters the narrower movement-driven presentation contract in [Sprite Root Motion](../../how-to/content/sprite-root-motion.md).

`ParticleBaker` owns both optional particle backends. It validates authored SPARK `.spark` XML and serializes deterministic `.spk` data; it compiles Effekseer `.efkproj` sources and dependencies into `.efk`. Both outputs include mandatory measured bounds, and authored `.spk`/`.efk` inputs are rejected. Use [Particle Format And Runtime](../../how-to/content/particle-format.md) and its generated [particle-format reference](../../reference/particle-format/index.md) for backend options, source/runtime formats, supported objects and render capabilities, incremental baking, runtime framing, integration routes, and the mandatory visible validation boundary.
...
Ogg/Theora decoder. Use [Video](../../how-to/content/video.md) and the generated
[video reference](../../reference/video/index.md) for whole-resource memory ownership,
fullscreen/embedded behavior, separate audio, diagnostics, and the mandatory
visible-client gate.
...
- `CompileManagedScripts` runs the standalone project `ManagedScriptBaker` after `ForceCodeGeneration` when `FO_MANAGED_SCRIPTING` is enabled. It compiles configured `.cs` inputs into resource-pack/target assemblies and generates the `.gen.cs`, `.gen.csproj`, and `.gen.sln` project surface; runtime payload preparation remains a separate packaging stage.
...
- Metadata baking: update `Source/Tools/MetadataBaker.*` and [Generated API and Metadata](../../reference/metadata/index.md).
- Prototype format/property loading: update `BuildTools/PrototypeFormatInterface.json`, [Prototype Format](../../how-to/content/prototype-format.md), regenerate its model/reference, run `test_docs_prototype_format.py`, and include it in the aggregate contract diff.
...
- Managed C# bake, generated-project, assembly, or runtime-payload behavior: update `Source/Tools/ManagedScriptBaker.*`, [Managed C# Scripting](../../how-to/scripting/managed-csharp.md), the native/Python managed tests, and the affected package/platform route.
...
7. Update this document and [BuildTools Pipeline](../../reference/cmake-and-buildtools/pipeline.md) if stage responsibilities change.
