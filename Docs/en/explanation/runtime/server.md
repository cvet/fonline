...
permalink: /Docs/en/explanation/runtime/server.html
---
...
- [Entity Model](../entity-and-property-model/) for entity/property/prototype ownership.
- [Maps, Movement, and Geometry](../maps-and-movement.md) for map positions, blockers, path finding, and movement contexts.
- [Networking](../authority-and-networking/) for command buffers, transports, and client/server message boundaries.
- [Persistence](../persistence/) for database backends and entity serialization.
- [Backup and Recovery](../../how-to/release/backup-and-recovery.md) for durable backup sets, oplog limits, isolated restore, and disaster-recovery evidence.
- [Client Runtime](client.md) for the client-side view of server messages.
- [Client Updater](client-updater.md) for the updater backend that is hosted by `ServerEngine`.
...
[Release Operations](../../how-to/release/operations.md) turns these startup/error/shutdown states, log markers, and optional health-file output into a deployment readiness and rollback procedure. It does not make process existence or an open listener equivalent to readiness.
...
`ServerEntity::_parent`, `Critter::_player`, and `Player::_controlledCr`. An
uncovered accessor cannot safely perform a bare pointer load followed by
`TryAddRef()`, because replacement could release the link's last reference in
between. Each link therefore has an `atomic_mutex` shared by load-plus-pin and
replacement. The old owner is released only after the new pointer is published.
A reader either pins the old target while the link still owns it or observes the
new target; it never tries to resurrect a freed object. `atomic_mutex` is used
because these accessors are `noexcept` and an OS mutex acquisition failure would
have no reporting path.

The controlled-critter link is non-owning. `Critter::DetachPlayer()` clears it
under the same lock before the critter can be destroyed, and `~Critter` verifies
the attachment is already gone. `SyncContext::SyncEntities()` keeps owning
handles for requested and widened entities, acquires the candidate cover, then
re-reads each parent/widen link under that cover. A concurrent reparent or
relink invalidates the candidate and triggers a bounded recomputation rather
than letting the target disappear between discovery and acquisition. Focused
coverage is in `ServerEngineSyncContextWidenAndAncestorCover`,
`ServerEngineSyncContextReparentStress`, and
...
Entity changes are persisted when relevant properties are saved by `ServerEngine::OnSaveEntityValue()` through `PropertiesSerializer`. The database facade and backends are documented in [Persistence](../persistence/).
...
For the wire-level model, see [Networking](../authority-and-networking/). For client behavior, see [Client Runtime](client.md).
...
The reusable geometry, path finding, blockers, line tracing, and map-loading concepts are documented in [Maps, Movement, and Geometry](../maps-and-movement.md). `MapManager` applies those concepts to authoritative server state.
...
Do not duplicate the Common entity taxonomy here; [Entity Model](../entity-and-property-model/) owns the base entity/property/prototype explanation.
...
`Source/Tests/Test_ServerEngine.cpp` includes overdue movement tests that verify route completion, condition-independent movement processing, and blocked-hex stopping behavior. Coordinate/pathfinding mechanics remain documented in [Maps, Movement, and Geometry](../maps-and-movement.md).
...
The client host/runtime updater flow is documented in [Client Updater](client-updater.md). Keep protocol-level details there and runtime hosting/ownership details here.
...
- Persistent state changes go through documented property/entity serialization boundaries from [Persistence](../persistence/).
- Client-originated data is validated before mutating authoritative state.
- New or changed network messages are cross-linked in [Networking](../authority-and-networking/) and [Client Runtime](client.md).
- Movement changes preserve [Maps, Movement, and Geometry](../maps-and-movement.md) invariants and server blocked-hex behavior.
- Script-facing events and methods are covered by server tests and the owning scripting documentation.
- Updater changes preserve the boundary between `UpdaterBackend` hosting here and client host/runtime behavior in [Client Updater](client-updater.md).
- Process-host changes preserve the operational lifecycle and evidence boundary in [Release Operations](../../how-to/release/operations.md).
