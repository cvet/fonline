...
| --- | --- | --- | --- |
| <code>Anim</code> | <code>Anim &lt;state&gt; &lt;action&gt; &lt;ModelFile&#124;animation-mesh&gt; &lt;clip&#124;~clip&#124;Base&gt;</code> | Maps a state/action pair to a source animation clip. ModelFile selects the primary model source, ~ reverses playback, and Base selects the first source clip before conversion into the baked runtime rig. | The first declaration for a pair is registered; model-specific lookup and substitutions are described in the canonical model-animation guide. |
