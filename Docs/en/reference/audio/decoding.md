...
| <a id="entry-audio-decoding-unsupported-extension-a56ebf6594"></a><code>audio.decoding.unsupported-extension</code> | Unsupported extension limitation | Author only wav, acm, or ogg paths. The current loader has no fallback rejection branch, so another explicit suffix bypasses all decoders and can enqueue an empty sound while returning success. | This is a documented implementation limitation, not support for arbitrary audio formats; callers and content validation must keep unsupported suffixes out. | [Source/Client/SoundManager.cpp](https://github.com/cvet/fonline/blob/master/Source/Client/SoundManager.cpp) |
