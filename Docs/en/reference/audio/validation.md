...
| <a id="entry-audio-validation-decoder-diagnostics-65927ba99d"></a><code>audio.validation.decoder-diagnostics</code> | Decoder diagnostics | Treat false playback results and RIFF, PCM, ACM, Ogg, or conversion log lines as authoring failures; reject unsupported suffixes in content validation because the runtime currently has no diagnostic for them. | The loaders report format-specific failure boundaries instead of substituting silence as a valid resource. | [Source/Client/SoundManager.cpp](https://github.com/cvet/fonline/blob/master/Source/Client/SoundManager.cpp) |
