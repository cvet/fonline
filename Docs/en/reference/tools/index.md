...
locale: en
document_id: tools
permalink: /Docs/en/reference/tools/
---
...
For CMake target creation and package wiring, see [Applications](../applications.md) and [BuildTools Pipeline](../cmake-and-buildtools/pipeline.md). For resource bake internals, see [Baking Pipeline](../../explanation/content-pipeline/baking.md).
...
Use it for the `CompileAngelScript` build path. The broader script runtime is documented in [Scripting](../../explanation/scripting-runtime/).
...
Use it for map editing, mapper-side automation, and headless mapper-based
workflows. See [Mapper Interactive Manual](../../how-to/tools/mapper-interactive.md) for the complete
human UI workflow and [Mapper Tools](../../how-to/tools/mapper.md) for lifecycle,
automation, and script helper details.

### Testing app

`Source/Applications/TestingApp.cpp` is the test runner entry point. Use the [native test-suite inventory](../../../../Source/Tests/README.md) and [Testing](../../contributing/testing/) for suite ownership.
...
[Video](../../how-to/content/video.md) for exact paths, authoring limits, whole-resource memory,
...
history, save discipline, and failure diagnosis. See
[Mapper Tools](../../how-to/tools/mapper.md) for lifecycle, extension points, and
automation/headless-render workflows.
...
Use [Viewer Tools](../../how-to/tools/animation-particle-viewers.md) for the exact build/launch commands,
AnimationViewer and ParticleViewer controls, persisted settings, review
workflows, failure diagnosis, and screenshot provenance. The current Engine
has no generic Editor or AssetExplorer application.
...
The operational workflow is documented in
[Particle Authoring Tools](../../how-to/tools/particle-authoring.md). The exact
source/runtime contract, SPARK object-handler coverage, save normalization,
preview dependencies, and project validation boundary are documented in
[Particle Format And Runtime](../../how-to/content/particle-format.md) and the generated
[particle tooling reference](../../../generated/particle-format/tooling.md).
Project-specific asset catalogs and visual-quality procedures remain in the
embedding project.
...
- Model-description syntax, source meshes, layers, attachments, materials, cuts, and runtime composition: `Source/Tools/ModelMeshBaker.*`, `Source/Tools/ModelInfoBaker.*`, [Model Format](../../how-to/content/model-format.md).
- Model animation tuples and effective-duration metadata: `Source/Tools/ModelInfoBaker.*`, [Model Animation](../../how-to/content/model-animation.md).
- Image-frame offsets and 2D walk/run alignment: `Source/Tools/ImageBaker.*`, `Source/Client/CritterHexView.*`, [Sprite Root Motion](../../how-to/content/sprite-root-motion.md).
- Full resource bake orchestration: `Source/Tools/Baker.*`, `Source/Applications/BakerApp.cpp`, [Baking Pipeline](../../explanation/content-pipeline/baking.md).
- Script bytecode compilation: `Source/Tools/AngelScriptBaker.*`, `Source/Applications/ASCompilerApp.cpp`, [Scripting](../../explanation/scripting-runtime/).
- Metadata tags and generated metadata resources: `Source/Tools/MetadataBaker.*`, [Generated API and Metadata](../metadata/index.md).
- Interactive map editing: `Source/Tools/Mapper.*`,
  `Source/Applications/MapperApp.cpp`, [Mapper Interactive Manual](../../how-to/tools/mapper-interactive.md).
- Headless mapper automation and script helpers: `Source/Tools/Mapper.*`,
  `Source/Scripting/MapperGlobalScriptMethods.cpp`, [Mapper Tools](../../how-to/tools/mapper.md).
...
- Effekseer authoring-tool build/staging and optional generic package include: `BuildTools/buildtools.py`, `BuildTools/EffekseerEditor/build.ps1`, `ThirdParty/Effekseer/Dev/Editor/`, and [BuildTools Pipeline](../cmake-and-buildtools/pipeline.md).
- Standalone animation/particle viewer shells: `Source/Tools/AnimationViewer.*`,
  `Source/Tools/ParticleViewer.*`, `Source/Applications/AnimationViewerApp.cpp`,
  `Source/Applications/ParticleViewerApp.cpp`, and
  [Viewer Tools](../../how-to/tools/animation-particle-viewers.md).
- Particle source formats, baking, and runtime factories:
  `Source/Tools/ParticleEditor.*`, [Particle Format And Runtime](../../how-to/content/particle-format.md).
...
4. For focused viewer changes, run
   `BuildTools/tests/test_docs_viewer_tools.py`, build and launch both viewer
   targets, and visibly execute the affected workflow. For particle UI
   changes, also validate the interactive Mapper path on the affected
   platform/backend.
...
6. If tool target wiring changes, inspect [Applications](../applications.md), [BuildTools Pipeline](../cmake-and-buildtools/pipeline.md), and the generated CMake targets from an embedding project.
