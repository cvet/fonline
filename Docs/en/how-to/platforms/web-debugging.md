...
- target-specific Managed assemblies and `ManagedRuntime/` class-library resources inside the preloaded resource pack when `FO_MANAGED_SCRIPTING` is enabled;
...
Managed Web uses the Mono interpreter and Engine JavaScript scheduling/entropy glue. Prove the Web-target assembly and prepared runtime, startup, callbacks/awaits, and teardown in the browser; a host `dotnet build` does not qualify the Wasm payload. See [Managed C# Scripting](../scripting/managed-csharp.md).
