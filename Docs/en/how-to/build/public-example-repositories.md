...
Run `python validate.py` for the native source/bake/process gate and `python validate.py --web` for the fast Web source/compile gate. `python validate.py --web-package` adds native-host force-baking, exact raw/ZIP payload validation, WebAssembly and `Resources.data` checks, and archive parity. After `npm ci` and `npx playwright install chromium` in `WebTests/`, `python validate.py --web-runtime` adds a native server, packaged HTTP delivery, required response and lifecycle markers, a real WebGL 2 context, console/page/network failure checks, and compositor-pixel evidence. These example routes are opt-in and are not part of the required Engine validation registry or workflow. On Windows, `cmake --build --preset windows-capture` launches the real Direct3D 11 client. On Linux, run `linux-capture` under Xvfb with software Mesa when Linux OpenGL evidence is needed; preserve the PNG, process report, exact Engine SHA, and backend record in the owning project workflow. Both native routes record twelve warmed samples, validate independent header/runtime/gallery/footer regions, and keep the strongest complete frame. `assets/provenance.json`, `quality/performance-budget.json`, and `captures/capture-contract.json` are release inputs: update them in the same change as any asset, layout, renderer, budget, or supported-backend change. Direct3D 11 and WebGL 2 captures are checked local backend evidence; Linux OpenGL is checked historical CI evidence from `cvet/fonline-content-showcase` run `30937990249`, whose artifact digest, PNG hash, process report, Engine pin, and pixel regions were independently verified before admission.
...
Content Showcase uses `web-packages web` for the Engine-owned Web prerequisites; any Xvfb/Mesa packages needed by its visual lane remain in the example repository's own workflow. Run the helper with the example-owned `Workspace` selected through `FO_WORKSPACE`; BuildTools derives the pinned `Workspace/emsdk` path and does not honor an unrelated `FO_EMSDK` override. Upload Linux OpenGL PNG, capture-contract, and process evidence before the longer WebGL 2 runtime lane so a later Web failure does not erase already valid backend evidence, while the overall required job remains failed until every lane passes.
