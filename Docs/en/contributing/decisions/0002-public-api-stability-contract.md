...
For project-native C++, `BuildTools/NativeExtensionInterface.json` is consumed by CMake/codegen and owns source roles, supported hook signatures/fallbacks/call sites, and binding rules. The domain is `experimental` and source-compatible only at a pinned engine revision; it does not promise a cross-revision binary ABI. Project implementations and external SDKs remain outside the engine contract.
