---
layout: default
title: Source Tree Guide
locale: en
document_id: source-tree
permalink: /Docs/en/contributing/source-tree/
---

# Source Tree Guide

This guide explains where to start when navigating `Source/`. It complements the shorter [Source README](../../../../Source/README.md).

## Quick routing

- Changing executable startup or target entry points: `Source/Applications/` and [Applications](../../reference/applications.md).
...
Route to the owning directory first. Before naming a concrete file, helper, or
...
Developer and build-time tools. Current tool files include baker classes,
config/effect/image/map/model/proto/text bakers, Mapper, AnimationViewer,
ParticleViewer, and the Mapper-hosted SPARK particle editor. The current tree
has no generic Editor or AssetExplorer implementation.
...
Particle authoring has a dedicated `ParticleBaker`: route `.spark`/`.spk`, `.efkproj`/`.efk`, backend options, SPARK registry/editor, Effekseer compilation, measured bounds, runtime framing, and client integration work through [Particle Format](../../how-to/content/particle-format.md).
...
Font descriptors likewise have no dedicated baker: route raw-copy settings through `RawCopyBaker`, referenced textures through `ImageBaker`, and `.fofnt`/`.fnt` parsing plus text layout through [Font Format](../../how-to/content/font-format.md).
