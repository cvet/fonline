...
permalink: /Docs/en/contributing/coding-contracts/thread-safety-analysis.html
---
...
> [ExceptionSafety.md](exception-safety.md).
...
| `fo::atomic_mutex` | atomic-state park/wake | short critical sections reached from `noexcept` code, where an OS mutex acquisition failure would have no reporting path |
...
`FO_TSA_ACQUIRED_BEFORE(...)`, `FO_TSA_ACQUIRED_AFTER(...)`, `FO_TSA_REQUIRES(...)`, `FO_TSA_REQUIRES_SHARED(...)`, `FO_TSA_ACQUIRE(...)`,
