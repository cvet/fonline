...
| `FO_WORKSPACE_CACHE` | Базовый URL для подготовленных workspace. Дерево MSVC SDK собирается один раз, хранится как `xwin-<version>-<arches>.tar.gz` и дальше скачивается целиком. |
...
`xwin` скачивает пакеты Microsoft сам, поэтому зеркалирование собственных загрузок движка его не
покрывает — именно поэтому кэш workspace хранит его *результат*.
