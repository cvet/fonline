...
<!-- docs-translation: {"document_id":"server-runtime","locale":"ru","source_path":"Docs/en/explanation/runtime/server.md","source_sha256":"3a629aec09ac3557ccc6d873faeed879b23c4811200ac3614798eb2d30e8aa23"} -->
...
- сохранение изменений сущностей и свойств через `DataBase` и `PropertiesSerializator`;
...
Ручные серверные методы сущностей, читающие или изменяющие своё состояние, объявляют предусловие LOCKED через `FO_VALIDATE_ENTITY(LOCKED, ...)` при входе. Макрос разворачивается в обычный выбрасывающий `ValidateEntityAccess(this)`, который при доступе без покрытия создаёт восстанавливаемый `ScriptException`. На границе задания или скрипта нарушение регистрируется, а задание продолжается, поэтому ошибка области синхронизации больше не останавливает весь сервер. Исключение, выходящее из метода с `noexcept`, по-прежнему завершает процесс, так что нарушения внутри таких аксессоров остаются фатальными; поверхность, достигающая перехватывающей границы, восстанавливается. Сгенерированные C++-аксессоры свойств (`GetX()`, `SetX()`, `IsNonEmptyX()`) проверяют владельца через `FO_VALIDATE_ENTITY_ACCESS_VALUE(entity)` в той же выбрасывающей форме и сейчас получают сущность из `Properties::GetEntity()` до обращения к хранилищу. Низкоуровневый сырой доступ `Properties` оставлен сериализации, загрузке, инструментам и путям с собственным контрактом доступа. Макросы находятся в `Common.h`, чтобы временные стабилизационные проверки позже можно было удалить или отключить централизованно. Пока многопоточная логика стабилизируется, проверка намеренно включена всегда. Методы, которыми пользуются сам валидатор, callback сохранения и механизм блокировок, явно помечены `FO_NO_VALIDATE_ENTITY_ACCESS()`: конструкторы и деструкторы, `GetId()`, `GetName()`, `IsPersistent()`, `GetEntityLock()`, сырой доступ к родителю, `GetSyncWidenEntity()` и низкоуровневые установщики родителя и блокировки не должны рекурсивно запускать проверку во время построения диагностики, обработки сохранения или доказательства текущего покрытия.
...
Изменения сущности сохраняются при записи соответствующих свойств методом `ServerEngine::OnSaveEntityValue()` через `PropertiesSerializator`. Фасад базы данных и реализации описаны в [сохранении данных](../persistence/).
...
Новое `NetworkServerConnection` входит в среду выполнения через `ServerEngine::OnNewConnection()` и превращается в неавторизованного `Player` через `CreateUnloginedPlayer()`.

Далее сервер обрабатывает игрока в двух основных состояниях:

1. **Неавторизованный игрок** — `ProcessUnloginedPlayer()` читает начальные протокольные сообщения и выполняет рукопожатие и вход.
...
Клиентские запросы движения поступают в `Process_Move()`, `Process_StopMove()` и `Process_Dir()`. Сервер проверяет запрос, вызывает скриптовые события `OnPlayerMoveCritter` и `OnPlayerDirCritter`, затем обновляет авторитетного `Critter` и рассылает получившееся состояние. Пакеты остановки содержат текущую клетку и смещение клиента. Сервер нормализует пару к канонической находящейся в границах клетке и смещению, согласует позиции на текущем авторитетном пути `MovingContext` криттера и допускает небольшую проверенную поиском пути коррекцию для быстрого старта и остановки между центрами пути. Это позволяет клиенту и серверу сходиться без разрешения произвольной телепортации при остановке. Если заявленную позицию согласовать нельзя, сервер останавливает криттера в своей авторитетной позиции и отправляет её управляющему игроку; только успешно согласованная остановка может пропустить лишнее самообновление.
