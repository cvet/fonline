...
Движок владеет защищённым каналом, транспортами, framing сообщений, упорядоченным UDP и интерфейсами соединений. Подключающий проект владеет топологией развёртывания, закреплёнными боевыми ключами, ротацией, эксплуатационной политикой и проектным использованием команд.
...
Каждое соединение, включая внутрипроцессный interthread-транспорт, передаёт упорядоченный поток шифрованных кадров до любого `NetMessage::Handshake` или трафика обновления. Движок реализует `Noise_NK_25519_ChaChaPoly_BLAKE2b` (Noise revision 34): до входа у клиента нет собственной криптографической личности, а сервер доказывает владение статическим секретом X25519, публичную половину которого клиент закрепил заранее. Одно рукопожатие выводит свежие ключи сессии для обоих направлений. Наблюдатель сети или поддельный сервер не может читать либо внедрять игровые сообщения и обновления. Владелец клиентского процесса всё ещё может извлечь ключи из памяти и посылать сообщения через настоящую сессию, поэтому серверная авторитетность и проверка входящих данных обязательны.

Один `SecureChannel` работает поверх TCP, упорядоченного UDP, WebSocket и interthread. Закрепление ключа WebSocket не зависит от PKI транспорта WSS. Поток сервера к клиенту сжимается до шифрования; поток клиента к серверу с логином и токенами не сжимается. Каждый кадр содержит 16-битную длину в big-endian и одно сообщение Noise. Полезная нагрузка ограничена 65 519 открытыми байтами и 16-байтным тегом. Недопустимая длина отклоняется до накопления тела. Заголовок упорядоченного UDP с номерами и подтверждениями остаётся вне канала, но не позволяет внедрить аутентифицированную нагрузку. Закрытие, переход UDP→TCP и переподключение создают новый канал; передачи открытым текстом в качестве запасного пути нет.

| Кадр | Отправитель | Содержимое |
|---|---|---|
| Offer | Клиент | Число от 1 до `SecureChannel::MAX_OFFERED_KEYS` (4) и первое сообщение NK для каждого закреплённого ключа сервера |
| Answer | Сервер | Индекс открытого предложения и второе сообщение NK; при отсутствии подходящего ключа сервер разрывает соединение |
| Transport | Любая сторона | Одно аутентифицированное транспортное сообщение Noise |

Оба сообщения рукопожатия имеют пустую нагрузку и фиксированный пролог `SecureChannel::PROLOGUE`. У каждого предложенного ключа собственный эфемерный ключ. Одновременное предложение старого и следующего ключей позволяет ротацию без второго рукопожатия. Счётчики nonce раздельны по направлениям; повторный, переставленный, потерянный или отражённый кадр нарушает аутентификацию. Ошибка канала — `NoiseException` либо производная `SecureChannelException` — закрывает соединение. Сервер фиксирует отказ на пути приёма и поручает своему worker отключить клиента с `ProtocolError`, записывая предупреждение, а не исключение движка для неаутентифицированного постороннего.

| Настройка | Сторона | Контракт |
|---|---|---|
| `ServerNetwork.ChannelSecretKey` | Сервер | Обязательный статический секрет X25519 из 64 шестнадцатеричных знаков даже при отключённой внешней сети; отсутствие или неверный формат не дают запустить сервер |
| `ClientNetwork.ChannelServerKeys` | Клиент | От одного до четырёх закреплённых публичных ключей по 64 шестнадцатеричных знака; пустой или неверный список не даёт подключиться |

`BuildTools/secure_channel_key.py generate <secret-file>` создаёт новый файл секрета только для владельца, не перезаписывает существующий и печатает публичный ключ; `public <secret-file>` выводит публичную половину уже существующего ключа. Боевой секрет нельзя запекать в клиент или конфигурацию репозитория: передавайте его на целевом хосте через `$TARGET_FILE{...}`, а клиентам публикуйте только pin. См. [Безопасность и секреты](../../how-to/release/security-and-secrets.md). При ротации сначала выпустите клиент с двумя ключами, затем смените секрет сервера и удалите старый pin после вывода старых клиентов. Тестовый fixture движка содержит собственную детерминированную пару, которая не является боевым ключом.

## Буферы сообщений

`Source/Common/NetBuffer.h` определяет общий бинарный слой сообщений:

- `NetBuffer` — общее хранилище, рост и сырое копирование; конфиденциальность и целостность обеспечивает `SecureChannel` вне буфера сообщений;
- `NetOutBuffer` — запись и framing исходящих сообщений;
- `NetInBuffer` — чтение и framing входящих сообщений.

Важная константа: `NETMSG_SIGNATURE = 0x011E9422`, маркер каждого сообщения внутри канала.
...
- предоставление `GetUnreadSize()` только для текущего framed message и `GetBufferedUnreadSize()` для всего сохранённого input;
- требование полного потребления текущего frame до того, как `ShrinkReadBuf()` или следующий `NeedProcess()` перейдёт дальше;
...
- **Проверка длины до выделения памяти.** Любая объявленная peer длина или количество проверяется относительно байтов, реально оставшихся в *текущем frame*, до выделения памяти или цикла. `NetInBuffer::Read<string>()` и `NetInBuffer::ReadPropsData()` выбрасывают `NetBufferException`, если длина больше `GetUnreadSize()`, поэтому маленькое сообщение не может заимствовать байты из следующего coalesced frame или спровоцировать многогигабайтное выделение. Декодирование входящего remote call использует read-only `DataReader`: байты строки сначала проверяются как заимствованный view, затем создаётся владеющая строка; количества массивов, словарей и словарей массивов сопоставляются с оставшимся payload по минимальному wire-размеру типа до `Reserve()`, создания контейнера или цикла по count. Серверный валидатор содержимого выполняет такую же предварительную проверку.
...
- **Максимум сохранённых входных данных.** `NetInBuffer::SetMaxBufLen(len)` ограничивает все непрочитанные байты в coalesced или частичных frames, поэтому peer не может бесконечно увеличивать receive buffer, не завершая сообщение. Сервер применяет `ServerNetwork.MaxBufferedInputSize` (по умолчанию 4 МиБ) к TCP, UDP, interthread и WebSocket input. Значение должно быть `0` (без ограничения) или не меньше `MaxMessageSize`, иначе запуск сервера завершается ошибкой. `AddData()` сообщает overflow в network thread, `ServerConnection` запоминает его один раз, а owning worker выполняет hard disconnect без повторного захвата transport lock.
- **Структурные пределы remote call.** Объявление `///@ RemoteCall` может задавать `MaxBytes N` и `MaxCollectionSize N`. Metadata несёт оба значения для каждого call: declaration без options запекается с явным trailer `Limits 0 0`, а registration отклоняет запись без него. Server разрешает имя call до allocation тела; native validator и decoder каждого включённого scripting backend проверяют collection limit до reserve или создания container. AngelScript покрывает arrays, dictionaries и nested dictionary arrays; текущий Managed C# bridge использует тот же wire reader, но поддерживает только scalar/array arguments. Неизвестный call отклоняется до allocation тела. Обязательный trailer изменил compatibility metadata и требует соответствующего увеличения compatibility version. Полная backend matrix приведена в [Remote Calls](../../reference/scripting/remote-calls.md).
- **Runtime-предел remote call.** `ServerNetwork.MaxRemoteCallPayloadSize` (по умолчанию 1 МиБ) задаёт общий серверный предел decoded RPC. Эффективный предел payload — меньшее ненулевое значение этой настройки и per-call `MaxBytes`; структурный предел вызова является семантической границей протокола, но не заменяет общий предел для враждебного input.
...
Посимвольный валидатор содержимого `ClientDataValidation.*`, вызываемый для клиентских записей свойств и входящих remote-call payload, дополняет эти слои. Он требует конечные float, корректный UTF-8, отсутствие встроенного NUL в строках — NUL допустим в UTF-8, но неприемлем в клиентском тексте и опасен для C-строк, журналов и БД, — неотрицательные размеры, согласованность count и payload, разрешаемые enum, hash и proto, а также соблюдение объявленного предела коллекций remote call. Абсолютные пределы input и флуда принадлежат верхнему слою буфера и транспорта.
...
### Слушатель, который не может занять порт, повторяется до отказа старта

Перезапуск состязается за порты с процессом, который он заменяет, и проигравший раньше валил весь
старт на первой же попытке: мир загрузился, освобождающийся за секунды сокет был ещё занят, и вся эта
работа выбрасывалась. Поэтому каждый удалённый слушатель запускается через
`ServerEngine::StartConnectionServer`, который повторяет попытки, пока не истечёт
`ServerNetwork.ListenRetryTime`, выжидая между ними `ServerNetwork.ListenRetryDelay`.

За крайним сроком исходное исключение перебрасывается и старт падает, потому что сервер, до которого
никто не достучится, — не запущенный сервер: повторы дают проигравшей стороне гонки немного времени,
но не превращают мёртвый порт в приемлемое состояние. Межпоточный транспорт сюда не входит: он не
занимает ничего, что мог бы держать другой процесс, поэтому отказ там — дефект, а не гонка.
