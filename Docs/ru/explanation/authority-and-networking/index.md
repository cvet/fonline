...
<!-- docs-translation: {"document_id":"networking","locale":"ru","source_path":"Docs/en/explanation/authority-and-networking/index.md","source_sha256":"4a520e213a18a7abb50e2c0ad51f57e48096c65e6b45322179e17689d48cd1ec"} -->
...
- **Структурные пределы remote call.** Объявление `///@ RemoteCall` может задавать `MaxBytes N` и `MaxCollectionSize N`. Метаданные несут оба значения для каждого вызова: декларация без options запекается с явным trailer `Limits 0 0`, а регистрация отклоняет запись без него. Сервер разрешает имя вызова до allocation тела; native validator и AngelScript decoder проверяют предел коллекции до `Reserve()` или создания контейнера, включая вложенные массивы словаря. Неизвестный вызов отклоняется до allocation тела. Обязательный trailer изменил compatibility metadata и требует соответствующего увеличения compatibility version.
