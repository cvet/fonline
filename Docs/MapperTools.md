...
The image produced here is only step 1. Step 2 (place entry-point markers, gather metadata) and step 3 (run the AI generator) will live as Python tooling under `../../Tools/` once the rendering side stabilizes. Both will read the PNG plus a JSON sidecar describing entry positions; emitting that JSON is the next addition to `MapperRender.fos`.
