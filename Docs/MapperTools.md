...
- **Name level** — a full-width horizontal line at the height where the critter name would sit: the top of the view rect shifted by the global `Settings->CritterView.NameOffset` **plus the prototype's own `NameOffset` property** (`proto->GetNameOffset()`), matching `CritterHexView::GetNameTextPos` (`Settings->CritterView.NameOffset + GetNameOffset()`).
...
- **View rect** — the visual/logical rect from `Sprite::GetViewSize()`, sourced from the model's baked **idle-pose** bounds (`_viewBounds`), not the live pose and not what the critter wears. It therefore stays put across the animation set (a strike can extend past it) and stays inside the draw rect, which is why name/UI text anchors to it — turn both on and the small box must never leave the big one. One deliberate exception: a clip whose baked box tops out **lower** than the idle box replaces it whole (`SelectModelViewBounds`), so a dead or prone critter carries its name at its own height instead of at standing height. Positioned bottom-centred as `MapSprite::GetViewRect` does.
...
**Attachments appear in that same tree**, as sub-nodes of the bone they connect to, labelled with the attached resource — an `AttachParticles` effect in cyan, a bone-linked child model (a weapon in a hand) in amber, matching the colour of its preview cross. Ticking one marks where it actually sits; hovering shows its bone, its authored `Move` and the resulting sprite pixel. So a mis-signed or mis-scaled `MoveX`/`MoveY`/`MoveZ` on the link is read directly off the preview instead of inferred from where the smoke drifts or the weapon looks to sit. Notes:

- A child model attached by *matching bone names* is **not** listed: it is not pinned to a point but follows the whole skeleton joint by joint, so there is nothing to mark.
- Nesting follows ownership — an effect hanging on a bone of an attached weapon appears under that weapon, not under a root bone.
- A particle point is the attachment bone's world position displaced by the authored offset **in the bone's own space**, exactly as `ModelInstance::ProcessAnimation` places the effect (`ModelParticleSystem::Move` → `ParticleSystem::Setup`'s position offset). A model point is the child's composed root placement (`GetWorldMatrix(parent joint) * _matTransBase * _matRotBase * _matScaleBase`), read from the matrix the pose pass already built rather than recomputed.
- Positions come from `ModelInstance::GetAttachPoints()`, which walks the whole hierarchy and projects every point through the **root** model's frame projection — only the root owns a frame — sharing the projection helper with the bone markers. An attachment with no usable frame yet is still listed, without a position.
...
The preview draws into its own offscreen `RenderTexture` (the Animation Viewer pattern, including `IsRenderTargetFlipped()` handling) and pins the effect **root** — bottom-centre adjusted by the sprite offset, `(size.width/2 - offset.x, size.height - offset.y)`, the same value `MapSprite::GetSpriteRootOffset()` uses — to a fixed anchor, so it stays put while the effect evolves. `Zoom` (slider + mouse wheel over the preview) magnifies the baked frame in the atlas path; in the scene path there is no baked frame, so it is pushed into the particle system as the effect's own scale (`ParticleSystem::SetScale`), which scales emitter placement and quad sizes together. **Holding the right mouse button and dragging pans** (the pan moves effect, crosshair, and overlays together, and resets on a new selection).

An effect previews through **its authored render path**, so the window shows what the game does: atlas-mode effects render through `Update()`→`DrawToAtlas()` then `DrawSpriteSize`; a `draw in scene = true` effect is drawn with `Sprite::DrawInScene` under a widened ortho depth range instead. The **Draw in scene** checkbox next to the play controls reflects that authored flag for the selected effect and flips it live (`ParticleSprite::SetDrawInScene`, the general engine switch between the two paths — flipping back to atlas re-applies the baked emitter transform). Use it to compare the paths: the atlas path clips the effect to its sprite frame and draws it as a flat quad, so overshoot and the real particle geometry only show in the scene path. The override is per-instance and resets to the authored value when another effect is selected; the resource is never modified.
...
- **Draw rect** — the sprite frame the effect rasterizes into in the atlas path (`Sprite::GetSize()`, derived from the baked bounds). It stays drawn in the scene path too, as a reference box: whatever reaches outside it there is what the atlas path would clip away.
- **Show wireframe** — magenta triangle edges of the actual particle geometry (every SPARK/Effekseer quad), drawn by the particle backends while `SpriteManager::IsDrawWireframe()` answers true; the viewer turns it on for its preview render and back off again, so a hosting mapper scene stays unaffected.
...
| `Game.SetMapperOverlayVisible(visible)` | Toggle mapper-only scroll-block markers. Embedding-project overlay scripts draw them through `Map.DrawMapSprite`, so off means clean previews; defaults to on for normal mapper use. |
...
6. Calls `Game.SaveMapperScreenshot("<RenderOutputDir>/<OutputName>.png")`, then unloads the current map and advances to the next. `OutputName` comes from `Mapper.RenderPlanOutputNames` when provided, otherwise from the loaded map name.
...
| `Mapper.RenderPlanOutputNames` | empty | Optional semicolon-separated batch list of screenshot base names, aligned with `Mapper.RenderMaps`. The preview tool uses this to load maps from subfolders while keeping `<MapPid>.png` output names. |
...
The image produced here is only step 1. Step 2 (place entry-point markers, gather metadata) and step 3 (run the AI generator) will live as Python tooling under `../../Tools/` once the rendering side stabilizes. Both will read the PNG plus a JSON sidecar describing entry positions; emitting that JSON is the next addition to `MapperRender.cs`.
...
| `--keep-raw` | off | Copy the uncropped mapper frame next to the preview for debugging. |
