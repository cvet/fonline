...
Configuration values remain strict UTF-8 from `ConfigFile` through config/sub-config storage and the `SetEntry` dispatch. A setting declared as ordinary `string` or `vector<string>` is an ASCII domain: assignment parses the existing setting grammar and then performs checked UTF-8-to-ASCII narrowing, so a non-ASCII value throws instead of leaving UTF-8 bytes in a character string. Settings that can contain user or platform text are declared as `u8string` / `vector<u8string>`; this includes the game/window name, command line and arguments, proxy credentials, Git branch text, Android credentials, and the database connection value. Physical paths are also `u8string`, including baking/resource/cache/platform-binary directories, the client writable root, database storage/oplog paths, and packaging input paths. Logical resource paths and names—including splash/effect/stub resources and the map-data prefix—are checked ASCII `string` values. Protocol tokens, language codes, setting names, and other technical identifiers remain checked ASCII as well. `ResourcePackInfo::InputDirs` / `InputFiles`, `SubConfigInfo::ConfigDir`, applied config paths, and saved setting values are strict UTF-8; resource-pack/sub-config names and baker names remain checked ASCII identifiers. `ApplyConfigAtPath()` accepts only UTF-8 path views, and `Save()` returns `map<string, u8string>`.
...
Disk text content is a separate strict domain: `fs_read_file_text()` validates and returns `u8string`, while `fs_write_file_text()` accepts strict `u8string_view` or an ASCII-domain `string_view`, copied byte-for-byte without an intermediate UTF-8 owner. Settings includes, file-list manifests, baker build hashes, JSON database files, generated fixtures, and project deep-link registration cross older parser/SDK APIs only through a bounded read-only character view owned by the live strict string. Invalid UTF-8 never becomes configuration or JSON text; callers that need arbitrary bytes must use the byte API.

Physical filesystem paths are strict UTF-8. `DataSource::MountDir()` / `MountPack()`, `FileSystem::AddDirSource()`, `FileHeader::GetDiskPath()`, `BakerDataSource` output paths, database storage paths, and every `DiskFileSystem` operation use `u8string` owners and `u8string_view` borrows. `DataSource::GetSourcePath()` returns the physical directory/archive path (or a synthetic source label for non-disk sources); `FileHeader::GetDiskPath()` combines it with the ASCII resource-relative path only for directory sources. Explicit physical archive names accepted by `DataSource::MountPack()` remain UTF-8, while the logical pack identifiers accepted by `FileSystem::AddPackSource()` / `AddPacksSource()` are ASCII and promote only inside that filesystem boundary.

Logical resource paths are a separate ASCII domain throughout `DataSource`, `FileSystem`, resource settings, sprite/effect/font lookup, and renderer-generated shader/pass names. They use `string` / `string_view`; there are no UTF-8 `FileSystem::ReadFile()` or `IsFileExists()` overloads. Directory enumeration narrows each UTF-8 filesystem-relative name with `utf8_to_string()`, and DAT/ZIP/embedded/file-list indices validate entries before publishing them. A non-ASCII filename may exist elsewhere on the physical filesystem, but it cannot become a mounted resource name. `std::filesystem::path` conversion remains centralized in `fs_make_path()` / `fs_path_to_u8string()` so invalid POSIX filename bytes cannot silently enter physical-path text.
