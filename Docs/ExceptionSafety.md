...
**A `NoThrow` classification does not by itself license a `noexcept` signature — the two diverge.** ES classification ignores `FO_VALIDATE_ENTITY` (above); the compiler does not. A method whose only throw is `FO_VALIDATE_ENTITY(… NOT_DESTROYING)` is `NoThrow` here, yet trips `C4297` if declared `noexcept` (§3.1). Add `noexcept` to a `NoThrow` function only when it *also* has **no reachable inline throw at all** — no `NOT_DESTROYING`, no hand-written `FO_VERIFY_AND_THROW` / `verify(…)`, no bare `throw`. (A method that legitimately needs such a guard while staying `noexcept` uses the `FO_VERIFY_AND_RETURN*` form instead — §5.) Conversely, a hand-written `FO_VERIFY_AND_THROW` / explicit `throw` that fires before any mutation makes a body `Strong`, not `NoThrow`, even when the guarded condition "can't happen" in valid operation: per the rule above such guards always count as throw points. The build's `C4297` (a `/W4` warning) is the backstop that catches a `noexcept` placed on a body that still throws.

**Also never `noexcept`: a C++ function registered with the AngelScript engine.** Since C++17 `noexcept` is part of a function's *type*, and AngelScript's `RegisterGlobalFunction` / `RegisterObjectMethod` / `RegisterObjectBehaviour` calling-convention detection rejects a `noexcept` function pointer at **registration time** with `asWRONG_CALLING_CONV` (a runtime `ScriptSystemException`, *not* a compile error — it builds clean and then every script call fails). This hits the manually-bound functions and generic wrappers in `Source/Scripting/AngelScript/*.cpp` (e.g. `Global_GetConstant<hstring>` behind `get_EMPTY_HSTRING`), so the AngelScript binding layer is excluded from `noexcept` wholesale. (Codegen `FO_SCRIPT_API` / `///@ ExportMethod` exports already take no `noexcept`.) The engine unit tests are the net here — `C4297` does not fire for these, and the build passes.
