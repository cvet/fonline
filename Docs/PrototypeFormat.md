...
Use this guide for the authoring model, inheritance, references, migrations, and validation workflow. Use the generated [syntax reference](generated/prototype-format/syntax.md), [built-in property catalog](generated/prototype-format/properties.md), [validation rules](generated/prototype-format/validation.md), and [canonical JSON model](generated/prototype-format.json) for exact declarations at the current engine revision.
...
Use `key = value` for replacement and `key += value` only where appending text is part of the property's documented representation. A backslash continues a logical line. `#` starts a comment outside quoted/escaped content.

Keys beginning with `$` control the prototype loader. Keys beginning with `_` are ignored by property application and should be reserved for project tooling with an explicit project contract. Every other key must resolve to metadata.
...
A property is temporary when it is mutable or core-owned and is not persistent. The generated [property catalog](generated/prototype-format/properties.md) applies this exact rule to current built-in metadata and lists the active sides.
...
The callback starts with the initialized entity covered. It must acquire or widen synchronization before accessing unrelated entities. See [ScriptLifecycleAndConcurrency.md](ScriptLifecycleAndConcurrency.md#entity-initscript-callbacks) for the runtime and concurrency contract.
...
- [BakingPipeline.md](BakingPipeline.md) - baker orchestration and output ownership.
- [EntityModel.md](EntityModel.md) - entity metadata and runtime identity.
- [ScriptLifecycleAndConcurrency.md](ScriptLifecycleAndConcurrency.md) - callback invocation, synchronization, failure, and teardown rules.
- [GeneratedApiAndMetadata.md](GeneratedApiAndMetadata.md) - source metadata and generated API models.
- [ApiChangeManagement.md](ApiChangeManagement.md) - aggregate contract diff and dispositions.
- [EmbeddingProject.md](EmbeddingProject.md) - reusable engine/project ownership boundary.
