...
Four kinds of name stay PascalCase inside the layer, in every case because the name is not Essentials' to choose:
...
Four implementations below the modules in the `Essentials.h` order keep their own OS calls, and that is structural rather than an oversight: `BasicCore.cpp` (debugger detection and the debugger break) is the first header in the order, so nothing can move out of it without inverting the layering; `BaseLogging.cpp` holds a single `SetConsoleOutputCP`; `StringUtils.cpp` owns the UTF-8/UTF-16 conversion, which is the string layer's own primitive; and `StackTrace.cpp` walks native stacks and names their frames (`RtlCaptureContext`, `RtlVirtualUnwind`, `StackWalk64` and DbgHelp on Windows, libunwind, libbacktrace and `dladdr` elsewhere), which the logging and fatal-error layers above it already need. The crash handlers themselves are installed through `winapi::install_crash_handlers` and `posix::install_crash_signal_handlers`. Two more stay outside for a different reason — they are OS wrappers themselves rather than consumers: `NetSockets.*` is the engine's BSD socket layer, and `ServerServiceApp.cpp` is a Windows service host whose contract with the OS is a set of callbacks, not a call list.
...
The standard string streams are specified on `std::basic_string`, so they keep the `stream_string` alias and text handed to one is copied through `make_stream_string`. `std::filesystem::path` likewise only recognises the standard string shapes; build a path from an engine string with `fs::make_path`, which is the correct engine idiom anyway because it carries UTF-8 rather than the native narrow encoding. The way back is `fs::path_to_string`, never `path.string()`: on Windows that converts through the ANSI code page, which throws `No mapping for the Unicode character exists in the target multi-byte code page` for a character the code page lacks and hands a code-page string to UTF-8 consumers otherwise - a writable root in a Cyrillic profile or install folder broke the managed runtime's assembly cache exactly this way.
...
Profiling builds allocate through the same rpmalloc, so a capture measures the allocator that ships. They only add accounting on top: per-thread allocation counters, and with the opt-in `Memory` profiling category a `TracyAlloc` / `TracyFree` per block. Tracy's private rpmalloc copy serves the profiler's own buffers and nothing else; the engine once allocated through it, which needed a `tracy::InitRpmalloc()` check on every allocation because that copy initializes per thread, while the engine rpmalloc initializes a thread lazily on its first allocation. Both copies declare the same attribute macros and spell three of them differently for GCC, so `MemorySystem.cpp` undefines those before including the engine header.
...
Not hooked, with reasons: **Monocypher** never allocates — every buffer is the caller's — so it has nothing to route. **LibreSSL** exports `CRYPTO_set_mem_functions` but its body is an inert `return 0;` — custom allocators were removed upstream, so calling it would be dead code that reads like coverage. **ogg / vorbis / theora** expose no allocator hook. **vkd3d-shader** exposes none either (`vkd3d_malloc` in `include/private/vkd3d_memory.h` is plain `malloc`), and runs only inside the effect baker, where every block belongs to one vkd3d-shader call (`vkd3d_shader_compile`, and the container parse and serialize in `Direct3DLevel9.cpp`) and is released through vkd3d's own free functions before `CompileHlsl` or `AddDirect3DLevel9Code` returns; an allocation failure there comes back as an error code and fails the bake instead of terminating it.
...
Windows named mutex). The two do not nest alike: a flock belongs to one open file description, so a second lock
