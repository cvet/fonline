...
The reverse adapters `utf8_to_char_span()`, `utf8_to_byte_span()`, and `string_to_byte_span()` are zero-copy read-only views. Their result is valid only while the source storage remains alive and unchanged. A UTF-8 raw C ABI pointer is available only through `utf8_to_c_str(u8string_view_nt)`; ordinary `string_view_nt::c_str()` serves the ASCII C ABI. Requiring an `_nt` view proves the trailing zero and absence of embedded zeros.
...
UTF-8 owning strings are accepted as formatting arguments only as lvalues; UTF-8 views and terminated views are also accepted. `strex`, `u8strex`, and `FormatUtf8()` accept ordinary narrow `string`, `string_view`, `std::string`, bounded `char` arrays, non-null `const char*`, and the matching proxy arguments. `u8strex` and `FormatUtf8()` additionally accept UTF-8 owners/views and UTF-8 proxies. Proxy arguments are adapted as bounded views for the duration of the outer formatting operation, so callers do not extract temporary strings. UTF-8 formatting privately adapts and validates the complete result. Raw wide text, raw `char8_t` pointers, UTF-8 owning rvalues, null character pointers, and dynamic format strings remain rejected.
...
`Test_TextFormatting.cpp` pins format-spec behavior, UTF-8 literals across Cyrillic, combining, and non-BMP text, ASCII-to-UTF-8 promotion, direct narrow-character promotion, embedded-zero preservation, and whole-result validation with exact failure diagnostics. Its compile-time gates accept strict owners and branded views with exact strict return types, accept bounded narrow-character values only at the UTF-8 entry point, and reject temporary strict owners, wide/raw `char8_t` text, dynamic formats, and every unclassified or UTF-8 argument at the ASCII entry point.
