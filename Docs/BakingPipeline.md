...
- `WasmBaker` — `Source/Tools/WasmBaker.*`, name `Wasm`, order `4`, enabled when `FO_WASM_SCRIPTING` is active; copies `.wasm` resources unchanged and processes `.fowasm` descriptors into metadata-derived frontend API manifests plus compiled `.wasm` resources. `WasmAssemblyScriptBaker.*` is the first language frontend helper and generates AssemblyScript declarations from those manifests during baking.
...
- WASM has no separate engine-owned compile target. When `FO_WASM_SCRIPTING` is enabled, projects can provide ready-to-bake `.wasm` files, or `.fowasm` descriptors that run an embedding-project frontend compiler during resource baking. `WasmBaker` generates `WasmApi/server.json`, `WasmApi/client.json`, or `WasmApi/mapper.json` from the current `EngineMetadata`, stages descriptor inputs, expands compiler placeholders such as `{api}`, `{source}`, `{output}`, `{bindings}`, and `{bindings_dir}`, then writes the compiler-produced `.wasm` and optional `.wasm.map` sidecar into baked resources. `Frontend = AssemblyScript` additionally invokes `WasmAssemblyScriptBaker` to write `fonline_api.ts` or the descriptor's `[AssemblyScript] Bindings` path from the manifest and, when no custom `Command` is supplied, defaults to `npm exec --yes --package assemblyscript -- asc {source} --outFile {output} --optimize`.

`.fowasm` descriptors are intentionally bake-time inputs, not codegen inputs. `BuildTools/codegen.py` still only generates C++ metadata and wrappers from engine `///@` annotations. The frontend compiler consumes the API JSON generated by `WasmBaker` from that metadata, so script authors can edit their WASM source files between bakes without rerunning codegen.
...
For Web packages with `FO_WASM_SCRIPTING` enabled, `BuildTools/package.py -wasm-scripting` performs an additional packaging step after resource preparation: client-visible `.wasm` files are copied from the baked client resource tree into `WasmScripts/`, and `WasmScripts/manifest.json` is generated from the modules' Type, Import, Function, and Export sections. The manifest includes function import signatures so the Web runtime can validate them against `Source/Scripting/Wasm/WasmImports.*`. This manifest is package output, not a baked source file.
...
- Script-specific bake behavior: update `Source/Tools/AngelScriptBaker.*`, `Source/Tools/WasmBaker.*`, `BuildTools/package.py` for Web WASM package manifests, and [Scripting.md](Scripting.md).
