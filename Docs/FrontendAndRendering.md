...
- **Immediate texture I/O is ordered against the deferred frame commands.** Draws and clears record into the frame command buffer (executed at present time), while `UpdateTextureRegion` / `GetTextureRegion` submit an immediate staging command buffer. Without extra ordering, an engine sequence like "clear a render-target atlas, then upload sprites into it" executes on the GPU as *upload first, clear last*, silently erasing the upload (this is exactly how texture-atlas sprites vanished: the atlas clear recorded earlier in the frame wiped every glyph/sprite uploaded after it). To preserve the immediate-mode semantics the other backends provide, both texture I/O paths first call `FlushFrameCommandBufferMidFrame()`, which submits the partially recorded frame command buffer (waiting the swapchain-acquire semaphore if this is the frame's first submit), waits idle, and restarts recording plus the current render pass. Keep this invariant when adding any new immediate-queue operation.
