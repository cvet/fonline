...
**Shader time is session-relative.** `TimeBuf` (`FrameTime.x` / `GameTime.x`, seconds) is rebased to the first rendered frame by `EffectManager::PerFrameEffectUpdate` — it is an animation-phase input, not an absolute clock. The raw steady-clock time is seconds since OS boot (days-scale on long-running machines), and such magnitudes collapse fp32 `fract()`/hash math in shaders into visible stepping artifacts (fp32 keeps only a few fractional bits past ~10^6). Effects that accumulate an unbounded phase from `FrameTime` (e.g. noise drift `t * speed` fed into a hash) must additionally wrap the phase locally — a periodic noise lattice with `mod(p, period)` keeps hash arguments small for any session length while staying seamless.
