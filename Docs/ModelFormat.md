...
Use this guide for the authoring and runtime model. Use the generated [token reference](generated/model-format/tokens.md), [asset and limit reference](generated/model-format/assets.md), [validation rules](generated/model-format/validation.md), and [canonical JSON model](generated/model-format.json) for the exact current-revision contract.
...
2. `ModelInfoBaker` runs at order `5`. It parses concrete `.fo3d` files, validates references and source freshness, loads the selected source skeletons/clips through the per-bake `ModelSourceAssetCache`, converts a canonical runtime rig, and writes versioned `LFMODINF` with a required `LFOZZRIG` payload at the same `.fo3d` path. It also emits `ModelAnimationInfo.foinfo` for common duration and bounds lookup.
...
Layer 8
Value 1
AttachParticles Particles/Jet.fopts Link Backpack
MoveY 0.15
RotY 90
```

The particle path is a global baked-resource path, not relative to the `.fo3d` file. Always provide a valid `Link` bone. Runtime particle creation requires it.
...
The attached resource's XML, registered SPARK objects, renderer fields, effects/textures, runtime cache, and visible validation are owned by [ParticleFormat.md](ParticleFormat.md). Model-bone particles use the direct 3D composition path rather than `ParticleSprite`'s atlas/direct-scene selector.
...
[BakingPipeline.md](BakingPipeline.md#shared-animation-metadata) and
[FrontendAndRendering.md](FrontendAndRendering.md#sprite-and-model-atlas-geometry) for the binary and
runtime contracts.
...
Use [ModelAnimation.md](ModelAnimation.md) for:
...
3D skeletal animation is separate from the 2D `NextX` / `NextY` contract in [SpriteRootMotion.md](SpriteRootMotion.md).
...
- animation tuples, aliases, speed, duration, or script lookups: update [ModelAnimation.md](ModelAnimation.md);
- 2D frame offsets or walk/run sprite phase: update [SpriteRootMotion.md](SpriteRootMotion.md);
