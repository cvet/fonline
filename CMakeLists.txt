...
set( FO_MONO_SOURCE "" )
set( FO_DEFAULT_CONFIG "" )
set( FO_DEBUGGING_CONFIG "" )
set( FO_MAPPER_CONFIG "" )
set( FO_GENERATE_ANGELSCRIPT_CONTENT "" )
set( FO_PACKAGES "" )
set( FO_APP_ICON "${CMAKE_CURRENT_SOURCE_DIR}/Resources/Radiation.ico" )
set( FO_RC_FILE "" )

# Evaluate build hash
if( NOT FONLINE_CMAKE_CONTRIBUTION STREQUAL "" )
	get_filename_component( FO_GIT_ROOT ${FONLINE_CMAKE_CONTRIBUTION} DIRECTORY )
else()
	set( FO_GIT_ROOT ${CMAKE_SOURCE_DIR} )
endif()
...
elseif( EMSCRIPTEN )
	...
	# add_link_options( -lhtml ) # Todo: won't link with -flto (check emscripten_compute_dom_pk_code_inlined in codebase)
...
set( FO_CLIENT_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/Client-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
set( FO_SERVER_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/Server-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
set( FO_SINGLE_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/Single-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
set( FO_EDITOR_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/Editor-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
set( FO_MAPPER_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/Mapper-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
set( FO_ASCOMPILER_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/ASCompiler-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
set( FO_BAKER_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/Baker-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
set( FO_TESTS_OUTPUT "${FONLINE_OUTPUT_PATH}/Binaries/Tests-${FO_BUILD_PLATFORM}$<${expr_PrefixConfig}:-$<CONFIG>>" )
...
macro( SetupGame )
	...
	set( allowedCommands "DEV_NAME" "NICE_NAME" "AUTHOR_NAME" "GAME_VERSION"
		...
		"DEFAULT_CONFIG" "DEBUGGING_CONFIG" "MAPPER_CONFIG" "GENERATE_ANGELSCRIPT_CONTENT" "GEOMETRY"
		"APP_ICON" )
	unset( opt )
	...
	foreach( arg ${ARGN} )
		...
		if( NOT optIndex EQUAL -1 )
			if( "${opt}" STREQUAL "GENERATE_ANGELSCRIPT_CONTENT" OR "${opt}" STREQUAL "APP_ICON" )
...
# Tracy profiler
StatusMessage( "+ Tracy" )
set( FO_TRACY_DIR "ThirdParty/tracy" )
add_compile_definitions( $<$<CONFIG:Profiling>:TRACY_ENABLE> )
...
configure_file( "BuildTools/blank.cmake" ${FO_RC_FILE} FILE_PERMISSIONS OWNER_WRITE OWNER_READ )
...
if( NOT FO_SINGLEPLAYER AND FONLINE_BUILD_CLIENT )
	if( NOT FO_BUILD_LIBRARY )
		...
		add_executable( ${FO_DEV_NAME}_Client WIN32 "Source/Applications/ClientApp.cpp" ${FO_RC_FILE} )
...
if( NOT FO_SINGLEPLAYER AND FONLINE_BUILD_SERVER )
	...
	add_executable( ${FO_DEV_NAME}_Server WIN32 "Source/Applications/ServerApp.cpp" ${FO_RC_FILE} )
...
if( FO_SINGLEPLAYER AND FONLINE_BUILD_SINGLE )
	if( NOT FO_BUILD_LIBRARY )
		...
		add_executable( ${FO_DEV_NAME} WIN32 "Source/Applications/SingleApp.cpp" ${FO_RC_FILE} )
...
if( FONLINE_BUILD_EDITOR )
	...
	add_executable( ${FO_DEV_NAME}_Editor WIN32 "Source/Applications/EditorApp.cpp" ${FO_RC_FILE} )
...
if( FONLINE_BUILD_MAPPER )
	...
	add_executable( ${FO_DEV_NAME}_Mapper WIN32 "Source/Applications/MapperApp.cpp" ${FO_RC_FILE} )
...
if( FONLINE_MAKE_EXTERNAL_COMMANDS )
	...
	cmake_path( RELATIVE_PATH CMAKE_CURRENT_SOURCE_DIR BASE_DIRECTORY ${FONLINE_OUTPUT_PATH} OUTPUT_VARIABLE engineDir )
	...
	configure_file( "BuildTools/blank.cmake" "${FONLINE_OUTPUT_PATH}/Starter.${scriptExt}" FILE_PERMISSIONS OWNER_EXECUTE OWNER_WRITE OWNER_READ )
endif()
