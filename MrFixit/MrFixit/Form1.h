...
namespace MrFixitEditor {
	...
	public:
		...
		void UpdateCraftsList()
		{
			...
			for(CraftItemMapIt it=crafts.begin(),end=crafts.end();it!=end;++it)
			{
				CraftItem* craft=(*it).second;
				String^ str=craft->Num+" - "+String(craft->Name.c_str()).ToString()+" - "+String(craft->Info.c_str()).ToString();
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->cbOr->Size = System::Drawing::Size(168, 17);
...
void ParseCraft(CraftItem* craft)
{
	...
	txtCraftName->Text=String(craft->Name.c_str()).ToString();
	txtCraftInfo->Text=String(craft->Info.c_str()).ToString();
	...
	for(size_t i=0,j=craft->ShowPNum.size();i<j;i++)
	{
		...
		String^ s2=String(s).ToString()+" "+param_val+"%";
	...
	for(size_t i=0,j=craft->NeedItems.size();i<j;i++)
	{
		...
		String^ s2=String(s).ToString()+" "+craft->NeedItemsVal[i]+"רע.";
	...
	for(size_t i=0,j=craft->NeedTools.size();i<j;i++)
	{
		...
		String^ s2=String(s).ToString()+" "+craft->NeedToolsVal[i]+"רע.";
	...
	for(size_t i=0,j=craft->OutItems.size();i<j;i++)
	{
		...
		String^ s2=String(s).ToString()+" "+craft->OutItemsVal[i]+"רע.";
		lbOutItems->Items->Add(s2);
	}

	// Other
	txtScript->Text=String(CurCraft->Script.c_str()).ToString();
